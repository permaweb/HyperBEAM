...
function isPojo(value) {
  return !isBytes(value) && typeof value === "object" && value !== null;
}
...
async function encode(obj = {}) {
  ...
  if (obj.type !== "Process") {
    obj = Object.keys(obj).reduce((acc, h2) => {
      if (h2 !== "device") {
        acc[h2] = obj[h2];
      }
      return acc;
...
// src/client/hb.js
async function encodeDataItem({ processId: processId2, data, tags, anchor }) {
  const obj = {};
  if (processId2) obj.target = processId2;
  if (anchor) obj.anchor = anchor;
  if (tags) tags.forEach((t) => {
    obj[t.name.toLowerCase()] = t.value;
  });
  obj.variant = "ao.N.1";
  obj.data = data;
...
function requestWith({ fetch: fetch2, logger: _logger, HB_URL, signer }) {
  ...
  return (fields) => {
    ...
    return of({ path: path2, method, fields: restFields }).chain(fromPromise(
      ({ path: path3, method: method2, fields: fields2 }) => {
        return encode(fields2).then(({ headers, body }) => ({
          path: path3,
          method: method2,
          headers,
          body
      ...
      async ({ path: path3, method: method2, headers, body }) => toHttpSigner(signer)(toSigBaseArgs({
      ...
      (request) => of(request).chain(fromPromise(
        ({ url, method: method2, headers, body }) => {
          return fetch2(url, { method: method2, headers, body, redirect: "follow" }).then(async (res) => {
...
function deployProcessWith({ fetch: fetch2, logger: _logger, HB_URL, signer }) {
  const logger = _logger.child("deployProcess");
  return (args) => {
    return of(args).chain(fromPromise(
      ({ processId: processId2, data, tags }) => encodeDataItem({ processId: processId2, data, tags })
...
function deployMessageWith({ fetch: fetch2, logger: _logger, HB_URL, signer }) {
  const logger = _logger.child("deployMessage");
  return (args) => {
    return of(args).chain(fromPromise(
      ({ processId: processId2, data, tags, anchor }) => encodeDataItem({ processId: processId2, data, tags, anchor })
    )).chain(fromPromise(({ headers, body }) => {
      return toHttpSigner(signer)(toSigBaseArgs({
      ...
      (request) => of(request).chain(fromPromise(
        ...
        (err) => Rejected(err),
        fromPromise(async ({ slot, processId: processId2 }) => {
          const { headers, body } = await encodeDataItem({ processId: processId2 });
          return toHttpSigner(signer)(toSigBaseArgs({
            url: `${HB_URL}/${processId2}/push&slot+integer=${slot}`,
            method: "POST",
            headers
...
function loadResultWith({ fetch: fetch2, logger: _logger, HB_URL, signer }) {
  const logger = _logger.child("loadResult");
  return (args) => {
    return of(args).chain(fromPromise(async ({ id, processId: processId2 }) => {
      const { headers, body } = await encodeDataItem({ processId: processId2 });
      headers.append("slot+integer", id);
      headers.append("accept", "application/json");
      return toHttpSigner(signer)(toSigBaseArgs({
        url: `${HB_URL}/${processId2}/compute&slot+integer=${id}/results/json`,
        method: "POST",
        headers
      })).then((req) => ({ ...req, body }));
...
function queryResultsWith({ fetch: fetch2, HB_URL, logger, signer }) {
...
var RedirectRequested = class extends Error {
...
function relayerWith({ fetch: fetch2, logger, HB_URL, signer }) {
  return async (url, options) => {
    ...
    return fetch2(hb, { ...options, headers: signedHeaders }).then((res) => {
      if (res.status === 400) {
...
// src/client/ao-mu.js
function deployMessageWith2({ fetch: fetch2, MU_URL: MU_URL2, logger: _logger }) {
  const logger = _logger.child("deployMessage");
  return (args) => {
    return of(args).chain(
      fromPromise(
        ({ processId: processId2, data, tags, anchor, signer }) => toDataItemSigner(signer)({ data, tags, target: processId2, anchor })
      )
    ).map(logger.tap("Successfully built and signed data item")).chain(
      (signedDataItem) => of(signedDataItem).chain(
        fromPromise(
          async (signedDataItem2) => fetch2(MU_URL2, {
            method: "POST",
            headers: {
...
function deployProcessWith2({ fetch: fetch2, MU_URL: MU_URL2, logger: _logger }) {
  const logger = _logger.child("deployProcess");
  return (args) => {
    return of(args).chain(
      fromPromise(
        ({ data, tags, signer }) => toDataItemSigner(signer)({ data, tags })
      )
    ).map(logger.tap("Successfully built and signed data item")).chain(
      (signedDataItem) => of(signedDataItem).chain(
        fromPromise(
...
function loadResultWith2({ fetch: fetch2, CU_URL: CU_URL2, logger }) {
  return ({ id, processId: processId2 }) => {
    return of(`${CU_URL2}/result/${id}?process-id=${processId2}`).map(logger.tap("fetching message result from CU")).chain(fromPromise(
...
function queryResultsWith2({ fetch: fetch2, CU_URL: CU_URL2, logger }) {
  return ({ process: process2, from, to, sort, limit }) => {
...
var getMessagesByRange = ({ fetch: fetch2, locate: locate2 }) => {
  return async ({ processId: processId2, from, to, limit }) => {
    const suUrl = (await locate2(processId2)).url;
    return fetch2(`${suUrl}/${processId2}?from=${from}&to=${to}&limit=${limit}`).then((res) => res.json());
  };
};
var getMessageById = ({ fetch: fetch2, locate: locate2 }) => {
  return async ({ processId: processId2, messageId }) => {
    const suUrl = (await locate2(processId2)).url;
    return fetch2(`${suUrl}/${messageId}?process-id=${processId2}`).then((res) => res.json());
  };
};
var getLastSlotWith = ({ fetch: fetch2, locate: locate2 }) => {
  return async ({ processId: processId2, since }) => {
    if (!since) {
      since = 10 * 60 * 1e3;
    }
    const from = Date.now() - since;
    const suUrl = (await locate2(processId2)).url;
    return fetch2(`${suUrl}/${processId2}?process-id=${processId2}&limit=100&from=${from}`).then((res) => res.json()).then((suResult) => {
...
function processIdWith(env) {
  const processId2 = env.processId;
  return (fields) => {
    fields = inputSchema3.parse(fields);
    return processId2(fields);
  };
}
...
// src/lib/request/transform.js
var transformToMap = (device) => (ctx) => {
  const result2 = ctx.res;
  const map3 = {};
  if (device === "relay@1.0") {
    if (typeof result2 === "string") {
      return result2;
    }
    if (result2.Output && result2.Output.data) {
      map3.Output = result2.Output;
    }
    if (result2.Messages) {
      map3.Messages = result2.Messages.map((m) => {
        const miniMap = {};
        m.Tags.forEach((t) => {
    ...
    if (typeof res.body === "string") {
      try {
        ...
        if (body.Output) {
          map3.Output = body.Output;
        }
        if (body.Messages) {
          map3.Messages = body.Messages.map((m) => {
            const miniMap = {};
            m.Tags.forEach((t) => {
              miniMap[t.name] = t.value;
            });
        ...
        map3.body = { text: () => Promise.resolve(body) };
      }
    }
    return map3;
  }
...
// src/lib/request/dispatch.js
function dispatch({ request, spawn: spawn2, message: message2, result: result2, dryrun: dryrun2, signer }) {
  return function(ctx) {
    ...
    if (ctx.type === "Message" && ctx.dataItem) {
      return fromPromise(
        (ctx2) => message2({
          process: ctx2.dataItem.target,
          anchor: ctx2.dataItem.anchor,
          tags: ctx2.dataItem.tags,
          data: ctx2.dataItem.data ?? "",
          signer
        }).catch((err) => {
          if (err.message.includes("Insufficient funds")) {
      ...
      )(ctx).chain(fromPromise((msg) => result2({ process: ctx.dataItem.target, message: msg }))).map((res) => ({ res }));
    }
    if (ctx.type === "Process" && ctx.dataItem) {
      return fromPromise(
        (ctx2) => spawn2({
          tags: ctx2.dataItem.tags,
          data: ctx2.dataItem.data,
          scheduler: ctx2.dataItem.tags.find((t) => t.name === "Scheduler")?.value,
          module: ctx2.dataItem.tags.find((t) => t.name === "Module")?.value,
          signer
    ...
    if (ctx.map) {
      return fromPromise((ctx2) => {
        return request(ctx2.map).then((res) => {
          return { res };
        });
      })(ctx);
    }
...
// src/lib/request/format.js
import { omit, keys } from "ramda";
var handleFormat = (mode, device) => (fields) => {
...
function requestWith2(env) {
  ...
  const message2 = env.message;
  const result2 = env.result;
  const dryrun2 = env.dryrun;
  const spawn2 = env.spawn;
  ...
  const verifyInput = (args) => {
    ...
    return of(inputSchema4.parse(args));
  };
  return (fields) => {
    const retry = (fn, request2, attempts) => fn(request2).bichain((err) => {
    ...
    const operation = (ctx) => verifyInput(ctx).map(handleFormat(mode, device)).chain(dispatch({ request, spawn: spawn2, message: message2, result: result2, dryrun: dryrun2, signer })).map((_) => {
      logger(
        'Received response from message sent to path "%s"',
        fields?.path ?? "/"
      );
      return _;
    }).map(transformToMap(device)).bimap(errFrom, identity);
    return retry(operation, { path: `/~${device}`, ...fields, method: fields.method ?? method }, 1).toPromise();
  };
}
...
function uploadMessageWith(env) {
  ...
  return (ctx) => {
    return of(ctx).chain(buildTags).chain(buildData).chain(fromPromise(
      ({ id, data, tags, anchor, signer }) => deployMessage({ processId: id, data, tags, anchor, signer: signerSchema.implement(signer || env.signer) })
...
function verifyInputWith2() {
  return (ctx) => {
    return of(ctx).map(inputSchema6.parse).map(() => ctx);
  };
}
...
function verifyInputWith3() {
  return (msg) => {
    return of(msg).map(inputSchema7.parse).map((m) => {
...
function getOperator(env) {
  return () => of({ URL: env.URL, fetch: env.fetch }).chain(getAddress).toPromise();
}
function getNodeBalance(env) {
  return () => of().chain(requestBalance(env.request)).toPromise();
}
...
function getAddress({ URL: URL2, fetch: fetch2 }) {
  return fromPromise(
    (nodeUrl) => fetch2(`${nodeUrl}/~meta@1.0/info/address`).then((res) => res.text()).catch((e) => "N/A")
  )(URL2);
}
...
function connectWith({ createDataItemSigner: createDataItemSigner3, createSigner: createSigner3 }) {
  ...
  function relayMode({
    ...
    if (!signer) {
      throw new Error("relay mode requires providing a signer to connect()");
    }
    const relayDataItemSigner = signer ? () => signer : createDataItemSigner3;
    const relaySigner = signer ? () => signer : createSigner3;
    ...
    const results2 = resultsWith({
      signer,
      queryResults: queryResultsWith2({
        fetch: fetch2,
        CU_URL: CU_URL2,
  ...
  function mainnetMode({
    ...
    logger("Mode Activated %s", isRelayMode ? "\u{1F500}" : "\u{1F432}");
    if (!signer) {
    ...
    const resultLogger = logger.child("result");
    const loadResult = isRelayMode ? loadResultWith2({
      fetch: relayFetch,
      logger: resultLogger,
      HB_URL: URL2,
      CU_URL: CU_URL2,
    ...
    const messageLogger = logger.child("message");
    const deployMessage = isRelayMode ? deployMessageWith2({
    ...
    const resultsLogger = logger.child("results");
    const queryResults = isRelayMode ? queryResultsWith2({
      fetch: relayFetch,
      CU_URL: CU_URL2,
      logger: resultsLogger
    }) : queryResultsWith({
    ...
    const spawnLogger = logger.child("spawn");
    const deployProcess = isRelayMode ? deployProcessWith2({
      fetch: relayFetch,
      HB_URL: URL2,
      MU_URL: MU_URL2,
      logger: spawnLogger
    ...
    const dryrun2 = dryrunWith({
      signer,
      dryrunFetch: dryrunFetchWith({
        fetch: isRelayMode ? relayFetch : fetch2,
        CU_URL: CU_URL2,
        logger: dryrunLogger
      }),
      logger: dryrunLogger
    ...
    return {
      ...
      spawn: spawn2,
      results: results2,
      getOperator: getOperator({ fetch: fetch2, URL: URL2 }),
