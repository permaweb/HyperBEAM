...
%% @doc Ask the manager to RST a stream and free its pool slot. Used when
...
handle_cast({cancel_stream, StreamRef},
    ...
    %% the connection remains usable for other callers.
    case maps:find(StreamRef, Streams) of
        {ok, {_CallerPid, ConnPid}} ->
...
terminate(_Reason, #state{authority = Authority, scope = Scope,
    ...
    maps:foreach(
        fun(StreamRef, {CallerPid, _ConnPid}) ->
...
drop_conn(ConnPid, ErrorReason, State = #state{conns = Cs, streams = Streams}) ->
    ...
    Streams2 = maps:filter(
        fun(Ref, {CallerPid, CConn}) when CConn =:= ConnPid ->
...
cancel_caller_streams(CallerPid, State = #state{streams = Streams,
                                                ...
                                                caller_refs = CRefs,
                                                queue = Q}) ->
    %% Streams owned by the dead caller.
    OwnStreams = maps:fold(
        fun(Ref, {CPid, ConnPid}, Acc) when CPid =:= CallerPid ->
            [{Ref, ConnPid} | Acc];
           (_, _, Acc) -> Acc
        end, [], Streams),
    %% Forget the stream locally so subsequent gun_* messages are dropped
    %% instead of forwarded. Do NOT send gun:cancel (RST_STREAM): under h2,
    %% late HEADERS/DATA already in flight race with the RST and gun emits
    %% stream_closed errors that can corrupt unrelated streams on the
    %% shared connection. Letting the stream drain naturally is safe since
    %% nothing is listening for its messages anymore.
    AffectedRefs = [R || {R, _} <- OwnStreams],
    Streams2 = maps:without(AffectedRefs, Streams),
    %% Decrement per-conn streams_in_flight for each RST'd stream.
    PerConnDec = lists:foldl(
        fun({_, ConnPid}, Acc) ->
            maps:update_with(ConnPid, fun(N) -> N + 1 end, 1, Acc)
        end, #{}, OwnStreams),
    Cs2 = maps:fold(
        fun(ConnPid, Dec, AccCs) ->
            case maps:find(ConnPid, AccCs) of
                {ok, Conn} ->
                    N = max(0, Conn#conn.streams_in_flight - Dec),
                    AccCs#{ConnPid => Conn#conn{streams_in_flight = N}};
                error -> AccCs
            end
        end, Cs, PerConnDec),
    ...
    case maps:take(CallerPid, CRefs) of
        {MonRef, CRefs2} ->
            ...
            drain_queue(State#state{streams = Streams2, conns = Cs2,
