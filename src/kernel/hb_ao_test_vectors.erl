...
denormalized_device_name_test(Opts) ->
    %% Devices are addressed by their public name. The runtime
    %% resolver translates the name to the generated `_hb_device_*'
    %% atom for `test-device@1.0'.
    Msg = #{ <<"device">> => <<"test-device@1.0">> },
    ?assertEqual(<<"test-device@1.0">>, hb_ao:get(device, Msg, Opts)),
    ?assertEqual(<<"test-device@1.0">>, hb_ao:get(<<"device">>, Msg, Opts)),
    {ok, ResolvedDev} = hb_ao_device:load(<<"test-device@1.0">>, Opts),
    {module, ActualMod} =
        erlang:fun_info(
            element(3, hb_ao_device:message_to_fun(Msg, test_func, Opts)),
            module
        ),
    ?assertEqual(ResolvedDev, ActualMod),
    ?assert(hb_packager:is_generated_module(ActualMod)).
