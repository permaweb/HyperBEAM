...
write(Opts, PathParts, Value) when is_list(PathParts) ->
    % Convert path segments to binary
...
%% @doc Join two path components together.
%%
%% This function concatenates two paths, handling various input formats (binary
%% or list of segments) and returning the result in binary form.
...
% Convert path to segments if it's not already
