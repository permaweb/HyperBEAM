...
script_open_fn(Script) ->
    script_open_fn(Script, http2).
...
caller_fifo_order_test_() ->
    {timeout, 10, fun() ->
        ...
        %% All 3 requests queue while conn is still connecting (delay_up fires
        %% after 150ms, by which time all 3 are enqueued).
