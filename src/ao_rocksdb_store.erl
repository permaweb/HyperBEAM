...
start_link(#{prefix := _Dir} = RocksDBOpts) ->
	gen_server:start_link({local, ?MODULE}, ?MODULE, RocksDBOpts, []);
start_link(_Opts) ->
	logger:error("Rocksdb server is not starting"),
	ignore.
...
resolve(_Opts, RawKey) ->
	Key = ao_store_common:join(RawKey),
...
do_resolve(CurrPath, [LookupKey | Rest]) ->
	LookupPath = ao_store_common:join([CurrPath, LookupKey]),
...
join(Key) when is_list(Key) ->
	KeyList = ao_store_common:join(Key),
