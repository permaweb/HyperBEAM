...
compute_slot(ProcID, State, RawInputMsg, InitReq, TargetSlot, Opts) ->
    ...
    PatchSourceUs = case erlang:erase(patch_source_us) of undefined -> 0; PS -> PS end,
    PatchFilterUs = case erlang:erase(patch_filter_us) of undefined -> 0; PF -> PF end,
    PatchReplaceUs = case erlang:erase(patch_replace_us) of undefined -> 0; PR -> PR end,
    PatchAccumUs = case erlang:erase(patch_accum_us) of undefined -> 0; PA -> PA end,
    PatchAccumCount = case erlang:erase(patch_accum_count) of undefined -> 0; PC -> PC end,
    PatchApplyUs = case erlang:erase(patch_apply_us) of undefined -> 0; PP -> PP end,
    MsgCommittedUs = case erlang:erase(dev_msg_committed_us) of undefined -> 0; MC -> MC end,
    MsgDeepMergeUs = case erlang:erase(dev_msg_deep_merge_us) of undefined -> 0; MD -> MD end,
    MsgSetCalls = case erlang:erase(dev_msg_set_calls) of undefined -> 0; MS -> MS end,
    MsgMaxMergeUs = case erlang:erase(dev_msg_max_merge_us) of undefined -> 0; MX -> MX end,
    MsgMaxMergeKeys = case erlang:erase(dev_msg_max_merge_keys) of undefined -> 0; MK -> MK end,
    MsgMatchUs = case erlang:erase(dev_msg_match_us) of undefined -> 0; MM -> MM end,
    MsgMaxKeyUs = case erlang:erase(dev_msg_max_key_us) of undefined -> 0; MKU -> MKU end,
    MsgMaxKey = case erlang:erase(dev_msg_max_key) of undefined -> none; MKK -> MKK end,
    TrieSetInnerUs = case erlang:erase(trie_set_inner_us) of undefined -> 0; TSI -> TSI end,
    TrieSetKeys = case erlang:erase(trie_set_keys) of undefined -> 0; TSK -> TSK end,
    RunAsSetupUs = case erlang:erase(run_as_setup_us) of undefined -> 0; RAS -> RAS end,
    RunAsExecUs = case erlang:erase(run_as_exec_us) of undefined -> 0; RAE -> RAE end,
    RunAsRestoreUs = case erlang:erase(run_as_restore_us) of undefined -> 0; RAR -> RAR end,
        ...
        {ok, NewProcStateMsg} ->
            ...
            ?event(compute_short,
                {computed_slot,
                    ...
                    {patch_source_ms, PatchSourceUs div 1000},
                    {patch_filter_ms, PatchFilterUs div 1000},
                    {patch_replace_ms, PatchReplaceUs div 1000},
                    {patch_accum_ms, PatchAccumUs div 1000},
                    {patch_accum_count, PatchAccumCount},
                    {patch_apply_ms, PatchApplyUs div 1000},
                    {msg_set_calls, MsgSetCalls},
                    {msg_committed_ms, MsgCommittedUs div 1000},
                    {msg_deep_merge_ms, MsgDeepMergeUs div 1000},
                    {msg_max_merge_ms, MsgMaxMergeUs div 1000},
                    {msg_max_merge_keys, MsgMaxMergeKeys},
                    {msg_match_ms, MsgMatchUs div 1000},
                    {msg_max_key_ms, MsgMaxKeyUs div 1000},
                    {msg_max_key, MsgMaxKey},
                    {trie_set_inner_ms, TrieSetInnerUs div 1000},
                    {trie_set_keys, TrieSetKeys},
                    {run_as_setup_ms, RunAsSetupUs div 1000},
                    {run_as_exec_ms, RunAsExecUs div 1000},
                    {run_as_restore_ms, RunAsRestoreUs div 1000},
