...
quantity_field(TX, Prefix, _Opts) ->
    case TX#tx.quantity of
        ?DEFAULT_QUANTITY -> #{};
        Quantity -> #{<<Prefix/binary, "quantity">> => integer_to_binary(Quantity)}
    end.

reward_field(TX, Prefix, _Opts) ->
    case TX#tx.reward of
        ?DEFAULT_REWARD -> #{};
        Reward -> #{<<Prefix/binary, "reward">> => integer_to_binary(Reward)}
    end.

data_root_field(TX, Prefix, _Opts) ->
    case TX#tx.data_root of
        ?DEFAULT_DATA_ROOT -> #{};
        DataRoot -> #{<<Prefix/binary, "data_root">> => hb_util:encode(DataRoot)}
    end.
