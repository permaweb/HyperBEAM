...
manifest_default_fallback_parallel_test() ->
...
manifest_404_error_parallel_test() ->
    Opts = #{
        <<"store">> => hb_opts:get(store, no_viable_store, #{}),
        <<"manifest-404">> => error
    },
...
manifest_inner_redirect_parallel_test() ->
    Opts = test_env_opts(),
    Node = hb_http_server:start_node(Opts),
...
access_key_path_in_manifest_parallel_test() ->
    Opts = test_env_opts(),
    Node = hb_http_server:start_node(Opts),
    ?assertMatch(
...
manifest_should_fallback_on_not_found_path_parallel_test() ->
    Opts = test_env_opts(),
    Node = hb_http_server:start_node(Opts),
    ?assertMatch(
        {ok, #{<<"commitments">> := #{<<"Tqh6oIS2CLUaDY11YUENlvvHmDim1q16pMyXAeSKsFM">> := _ }}},
