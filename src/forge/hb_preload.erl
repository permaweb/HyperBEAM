...
%%% </ul>
-module(hb_preload).
-export([build/3, build_groups/4, build_dir/4, write_index_header/2]).
...
build_dir(Pkgs, Wallet, OutputDir, Opts) ->
    ...
    ?event(preload, {build_start, {output, OutputBin}, {devices, length(Pkgs)}}),
    StoreCfg =
        #{
