%%% @doc Session manager for llama.cpp device to handle session history across processes.
%%%
...
-module(llamacpp_session_manager).
...
init([]) ->
    ...
    ets:new(llamacpp_sessions_lru, [named_table, ordered_set, public]),
    % Create ETS table for LRU timestamps
    ets:new(llamacpp_session_timestamps, [named_table, ordered_set, public]),
    {ok, #state{}}.
...
handle_call({get_session_history, SessionId, Prompt}, _From, State) ->
    ...
    History = case ets:lookup(llamacpp_sessions_lru, SessionId) of
        [] -> 
            % New session
            [];
        [{SessionId, ExistingHistory}] -> 
            % Update timestamp for LRU
            ets:delete(llamacpp_session_timestamps, {get_session_timestamp(SessionId), SessionId}),
            ExistingHistory
    end,
    ...
    ets:insert(llamacpp_sessions_lru, {SessionId, TrimmedHistory}),
    ets:insert(llamacpp_session_timestamps, {{Timestamp, SessionId}, TrimmedHistory}),
...
handle_call({update_session_history, SessionId, _UserPrompt, AssistantResponse}, _From, State) ->
    ...
    case ets:lookup(llamacpp_sessions_lru, SessionId) of
        [] -> 
            % Create new session with just the assistant response
            NewHistory = [#{<<"role">> => <<"assistant">>, <<"content">> => AssistantResponse}],
            ets:insert(llamacpp_sessions_lru, {SessionId, NewHistory}),
            ets:insert(llamacpp_session_timestamps, {{Timestamp, SessionId}, NewHistory});
        [{SessionId, ExistingHistory}] ->
            ...
            ets:delete(llamacpp_session_timestamps, {OldTimestamp, SessionId}),
            ets:insert(llamacpp_session_timestamps, {{Timestamp, SessionId}, NewHistory})
    end,
    {reply, ok, State}.
...
terminate(_Reason, _State) ->
    % Clean up ETS tables
    ets:delete(llamacpp_sessions_lru),
    ets:delete(llamacpp_session_timestamps),
    ok.
...
get_session_timestamp(SessionId) ->
    case ets:match(llamacpp_session_timestamps, {{'$1', SessionId}, '_'}) of
...
evict_old_sessions() ->
    SessionCount = ets:info(llamacpp_sessions_lru, size),
    if 
...
evict_oldest_sessions(Count) ->
    OldestSessions = ets:first(llamacpp_session_timestamps),
...
evict_oldest_sessions(Count, {Timestamp, SessionId}, Evicted) ->
    ...
    ets:delete(llamacpp_sessions_lru, SessionId),
    ets:delete(llamacpp_session_timestamps, {Timestamp, SessionId}),
    
    % Move to next
    NextKey = ets:next(llamacpp_session_timestamps, {Timestamp, SessionId}),
...
session_manager_test() ->
    ...
    % Test getting session history for new session
    SessionId = <<"test-session">>,
    Prompt = <<"Hello">>,
    History = get_session_history(SessionId, Prompt),
    Expected = [#{<<"role">> => <<"user">>, <<"content">> => <<"Hello">>}],
    ?assertEqual(Expected, History),
    
    % Test updating session history
    Response = <<"Hi there!">>,
    ok = update_session_history(SessionId, Prompt, Response),
    
    % Test getting updated history
    NewPrompt = <<"How are you?">>,
    UpdatedHistory = get_session_history(SessionId, NewPrompt),
    ExpectedUpdated = [
