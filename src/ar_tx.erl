...
json_struct_to_tx_happy_test() ->
    ...
    Variants = [
        ...
        {hb_ao:explicit_set(BaseStruct0, #{ <<"format">> => 2 }), fun(TX) -> ?assertEqual(2, TX#tx.format) end},
        {hb_ao:explicit_set(BaseStruct0, #{ <<"format">> => <<"2">> }), fun(TX) -> ?assertEqual(2, TX#tx.format) end},
        {hb_ao:explicit_set(BaseStruct0, #{ <<"quantity">> => <<"1234">> }), fun(TX) -> ?assertEqual(1234, TX#tx.quantity) end},
        {hb_ao:explicit_set(BaseStruct0, #{ <<"reward">> => <<"567">> }), fun(TX) -> ?assertEqual(567, TX#tx.reward) end},
        {hb_ao:explicit_set(BaseStruct0, #{ <<"tags">> => [TagFun(<<"tagname">>, <<"tagval">>)] }),
            fun(TX) -> ?assertEqual([{<<"tagname">>, <<"tagval">>}], TX#tx.tags) end},
        {hb_ao:explicit_set(BaseStruct0, #{ <<"denomination">> => <<"7">> }), fun(TX) -> ?assertEqual(7, TX#tx.denomination) end},
        {hb_ao:explicit_set(BaseStruct0, #{ <<"target">> => TargetB64 }),
            fun(TX) -> ?assertEqual(TargetBin, TX#tx.target)end},
        {hb_ao:explicit_set(BaseStruct0, #{ <<"data_root">> => hb_util:encode(DataRoot32) }),
            fun(TX) -> ?assertEqual(DataRoot32, TX#tx.data_root) end},
        {hb_ao:explicit_set(BaseStruct0, #{ <<"data_size">> => <<"250">> }),
            fun(TX) -> ?assertEqual(250, TX#tx.data_size) end},
        {hb_ao:explicit_set(BaseStruct0, #{ <<"signature">> => hb_util:encode(Sig64) }),
...
json_struct_to_tx_failure_test() ->
    ...
    FailureCases = [
        {"invalid_format", hb_ao:explicit_set(BaseStruct, #{ <<"format">> => <<"abc">> }), badarg},
        {"denomination_zero", hb_ao:explicit_set(BaseStruct, #{ <<"denomination">> => <<"0">> }), {badmatch, false}},
        {"id_wrong_size", hb_ao:explicit_set(BaseStruct, #{ <<"id">> => hb_util:encode(BadIDBin) }), {badmatch, 31}},
        {"quantity_nonnumeric", hb_ao:explicit_set(BaseStruct, #{ <<"quantity">> => <<"notanumber">> }), badarg},
        {"reward_nonnumeric", hb_ao:explicit_set(BaseStruct, #{ <<"reward">> => <<"xyz">> }), badarg},
        {"data_size_nonnumeric", hb_ao:explicit_set(BaseStruct, #{ <<"data_size">> => <<"abc">> }), badarg},
        {"quantity_missing", maps:remove(<<"quantity">>, BaseStruct), badarg},
        {"id_invalid_b64", hb_ao:explicit_set(BaseStruct, #{ <<"id">> => InvalidB64 }), badarg},
        {"owner_invalid_b64", hb_ao:explicit_set(BaseStruct, #{ <<"owner">> => InvalidB64 }), badarg},
        {"last_tx_invalid_b64", hb_ao:explicit_set(BaseStruct, #{ <<"last_tx">> => InvalidB64 }), badarg},
        {"signature_invalid_b64", hb_ao:explicit_set(BaseStruct, #{ <<"signature">> => InvalidB64 }), badarg},
        {"data_invalid_b64", hb_ao:explicit_set(BaseStruct, #{ <<"data">> => InvalidB64 }), badarg},
        {"data_root_invalid_b64", hb_ao:explicit_set(BaseStruct, #{ <<"data_root">> => InvalidB64 }), badarg},
        {"tag_name_invalid_b64", hb_ao:explicit_set(BaseStruct, #{ <<"tags">> => BadTagName }), badarg},
        {"tag_value_invalid_b64", hb_ao:explicit_set(BaseStruct, #{ <<"tags">> => BadTagValue }), badarg},
        {"target_invalid_b64", hb_ao:explicit_set(BaseStruct, #{ <<"target">> => InvalidB64 }), badarg}
        ],
