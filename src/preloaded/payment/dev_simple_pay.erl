...
%%% 2. If the request matches one of the `router-opts/offered' routes, the
...
-spec charge(
    #{ _ => _ },
    #{ request => #{ _ => _ }, quantity => integer(), _ => _ },
    #{ _ => _ }
) ->
    {ok, boolean()}
...
-spec topup(
    #{ _ => _ },
    #{ amount => integer(), recipient => binary(), _ => _ },
    #{ _ => _ }
