...
-device_libraries([
    lib_arweave_scheduler_cache,
    lib_arweave_scheduler_sync,
    lib_arweave_tx,
    lib_process
]).
...
next(Base, Req, Opts) ->
    ...
    maybe
        {ok, _} ?= lib_arweave_scheduler_sync:process(ProcessID, Opts),
        {ok, Assignment} ?=
...
find_assignment(ProcessID, Slot, Opts) ->
    case lib_arweave_scheduler_cache:read_assignment(ProcessID, Slot, Opts) of
...
schedule(Base, Req, Opts) ->
    case scheduler_method(hb_maps:get(<<"method">>, Req, <<"GET">>, Opts)) of
        ...
        _ ->
            {error,
                #{
                    <<"status">> => 405,
                    <<"reason">> => <<"Scheduler supports only GET and POST.">>
                }
            }
    end.

scheduler_method(Method) when is_binary(Method) ->
    try hb_util:to_lower(Method) of
        <<"get">> -> get;
        <<"post">> -> post;
        _ -> unsupported
    catch
...
get_schedule(Base, Req, Opts) ->
    ...
    maybe
        {ok, #{ <<"next-slot">> := NextSlot }} ?=
            lib_arweave_scheduler_sync:process(ProcessID, Opts),
        Latest = NextSlot - 1,
        To =
            case RequestedTo of
                undefined -> Latest;
        ...
        CappedTo = min(To, From + ?MAX_ASSIGNMENT_QUERY_LEN - 1),
        {ok, Assignments} ?=
            read_assignments(ProcessID, From, CappedTo, Opts),
        {ok, Bundle} ?=
            lib_arweave_scheduler_cache:assignments_to_bundle(
                ProcessID,
                Assignments,
                CappedTo < To,
                Opts
...
post_schedule(Base, Req, Opts) ->
    ...
    maybe
        ...
        Targets = lib_arweave_scheduler_sync:targets(TX),
...
read_assignments(_ProcessID, From, To, _Opts) when From > To -> {ok, []};
read_assignments(ProcessID, Slot, To, Opts) ->
    case lib_arweave_scheduler_cache:read_assignment(ProcessID, Slot, Opts) of
        {ok, Assignment} ->
            maybe
                {ok, Rest} ?=
                    read_assignments(ProcessID, Slot + 1, To, Opts),
                {ok, [Assignment | Rest]}
            end;
        not_found ->
            {error,
                #{
                    <<"status">> => 500,
                    <<"reason">> => <<"Scheduler assignment frontier is incomplete.">>,
                    <<"slot">> => Slot
                }
            };
        Error -> Error
    ...
    case Raw of
        ...
        _ ->
            try {ok, hb_cache:ensure_all_loaded(Raw, Opts)}
                ...
                throw:{necessary_message_not_found, _, _} -> missing_message();
                throw:{could_not_read_lazy_link, _, _, _} -> missing_message()
            end
    end.
