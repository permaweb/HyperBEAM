...
validate_next_slot(Base, [RawNextAssignment|RawAssignments], Lookahead, Last, Opts) ->
    NextAssignment = prepare_assignment_for_execution(RawNextAssignment, Opts),
    Assignments =
        lists:map(
            fun(Assignment) -> prepare_assignment_for_execution(Assignment, Opts) end,
            RawAssignments
        ),
...
prepare_assignment_for_execution(Assignment, Opts) ->
    Loaded =
        hb_cache:read_all_commitments(
            hb_cache:ensure_all_loaded(Assignment, Opts),
            Opts
        ),
    case cached_assignment_for_same_slot(Loaded, Opts) of
        {ok, Cached} ->
            hb_cache:read_all_commitments(
...
cached_assignment_for_same_slot(Assignment, Opts) ->
    try
        ProcID = hb_ao:get(
            <<"process">>,
            Assignment,
