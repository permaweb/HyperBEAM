...
-export([schedule/3, router/4, post_transfer/4]).
%%% CU-flow functions:
...
next(Base, Req, Opts) ->
    ...
    {Epoch, LastProcessed} = scheduler_cursor(Base, Opts),
...
%% @doc Return the scheduler epoch and slot cursor from a process state.
scheduler_cursor(Base, Opts) ->
    CursorOpts = Opts#{ <<"hashpath">> => ignore },
    {
        hb_util:int(hb_ao:get(<<"scheduler-epoch">>, Base, <<"0">>, CursorOpts)),
        hb_util:int(
            hb_ao:get(
                <<"scheduler-slot">>,
                Base,
                hb_ao:get(<<"at-slot">>, Base, CursorOpts),
                CursorOpts
            )
        )
    }.
...
validate_next_slot(
    ...
    NextAssignmentSlot = assignment_int(<<"slot">>, NextAssignment, not_found, Opts),
    ?event(next_profiling, found_next_assignment_slot),
    NextAssignmentEpoch =
        assignment_int(<<"epoch">>, NextAssignment, <<"0">>, Opts),
    ...
    ?event(debug_next,
        {calculating_next_from_assignments,
            {last_processed, Last},
            {epoch, Epoch},
            {next_epoch_from_assignment, NextAssignmentEpoch},
            {next_slot_from_assignment, NextAssignmentSlot},
            {assignments_received, length(Assignments)}
        }),
    ExpectedSlot = Last + 1,
    case {NextAssignmentEpoch, NextAssignmentSlot} of
        {not_processable, _} ->
            {error,
                #{
                    <<"status">> => 500,
                    <<"reason">> =>
                        <<"Unprocessable epoch value received in assignment.">>
                }
            };
        {UnexpectedEpoch, _} when UnexpectedEpoch =/= Epoch ->
            {error,
                #{
                    <<"status">> => 404,
                    <<"reason">> =>
                        <<"Received assignment epoch does not match expected epoch.">>,
                    <<"unexpected-epoch">> => UnexpectedEpoch,
        ...
        {Epoch, ExpectedSlot} ->
            ...
            case transfer_next_scheduler(NextAssignment, Opts) of
                {error, Error} ->
                    {error, Error};
                Transfer ->
                    {CacheAssignments, CacheLookahead} =
                        case Transfer of
                            false -> {Assignments, Lookahead};
                            {transfer, _} -> {[], undefined}
                        end,
                    NextState =
                        case hb_util:atom(
                            hb_opts:get(scheduler_in_memory_cache, true, Opts)
                        ) of
                            true ->
                                hb_private:set(
                                    Base,
                                    #{ <<"scheduler@1.0">> => #{
                                        ...
                                        <<"lookahead-worker">> => CacheLookahead
                                    }},
                                    Opts
                                );
                            false ->
...
%% @doc Return an integer assignment key, or `not_processable'.
assignment_int(Key, Assignment, Default, Opts) ->
    try hb_util:int(
        hb_ao:get(Key, Assignment, Default, Opts#{ <<"hashpath">> => ignore })
    )
    catch
        error:badarg -> not_processable
    end.

%% @doc Return the next scheduler from a terminal transfer marker.
transfer_next_scheduler(Assignment, Opts) ->
    NextScheduler = hb_ao:get(<<"next-scheduler">>, Assignment, not_found, Opts),
    BodyNextScheduler =
        hb_ao:get_first(
            [
                {Assignment, <<"body/next-scheduler">>},
                {Assignment, <<"body/Next-Scheduler">>}
            ],
            not_found,
            Opts
    ...
    Transfer =
        lists:any(
            fun(Key) ->
                lists:member(
                    hb_util:to_lower(hb_ao:get(Key, Assignment, <<>>, Opts)),
    ...
    case {Transfer, NextScheduler, BodyNextScheduler} of
        {true, not_found, _} ->
            {error,
                #{
                    <<"status">> => 400,
        ...
        {true, _, not_found} ->
            {error,
                #{
                    <<"status">> => 400,
                    <<"body">> =>
                        <<"Scheduler-Transfer body missing next-scheduler.">>
                }
            };
        {true, NextScheduler, NextScheduler} ->
        ...
        {true, _, _} ->
            {error,
                #{
                    <<"status">> => 400,
                    <<"body">> =>
                        <<"Scheduler-Transfer next-scheduler mismatch.">>
                }
            };
        {false, not_found, _} ->
            false;
        {false, _, _} ->
            {error,
                #{
                    <<"status">> => 400,
                    <<"body">> =>
                        <<"Scheduler-Transfer missing transfer action or type.">>
                }
            }
    end.
...
find_next_assignment(_Base, _Req, Schedule = [_Next|_], _Epoch, _LastSlot, _Opts) ->
    {ok, Schedule, undefined};
find_next_assignment(Base, Req, _Schedule, Epoch, LastSlot, Opts) ->
...
%% @doc Spawn a process to fetch the next epoch assignment from local cache.
spawn_lookahead_worker(ProcID, Epoch, Slot, Opts) ->
    Caller = self(),
    spawn(
        fun() ->
            ...
            case dev_scheduler_cache:read(ProcID, Epoch, Slot, Opts) of
                {ok, Assignment} ->
                    ...
                    Caller ! {assignment, ProcID, Epoch, Slot, LoadedAssignment};
                not_found ->
                    fail
            end
        end
...
%% option.
check_lookahead_and_local_cache(Base, ProcID, Epoch, TargetSlot, Opts)
        when is_map(Base) ->
...
check_lookahead_and_local_cache(Worker, ProcID, Epoch, TargetSlot, Opts)
        when is_pid(Worker) ->
    receive
        {assignment, ProcID, OldEpoch, _OldSlot, _Assignment}
                when OldEpoch < Epoch ->
            check_lookahead_and_local_cache(Worker, ProcID, Epoch, TargetSlot, Opts);
        {assignment, ProcID, Epoch, OldSlot, _Assignment} when OldSlot < TargetSlot ->
            ...
            check_lookahead_and_local_cache(Worker, ProcID, Epoch, TargetSlot, Opts);
        {assignment, ProcID, Epoch, TargetSlot, Assignment} ->
            ...
            NewWorker = spawn_lookahead_worker(ProcID, Epoch, TargetSlot + 1, Opts),
            ?event(next_lookahead,
    ...
    after ?LOOKAHEAD_TIMEOUT ->
        ...
        check_lookahead_and_local_cache(undefined, ProcID, Epoch, TargetSlot, Opts)
    end;
check_lookahead_and_local_cache(undefined, ProcID, Epoch, TargetSlot, Opts) ->
    ...
    case dev_scheduler_cache:read(ProcID, Epoch, TargetSlot, Opts) of
        not_found -> not_found;
        {ok, Assignment} ->
            ...
            Worker =
                case hb_opts:get(scheduler_lookahead, true, Opts) of
                    ...
                    true ->
                        ...
                        % ahead of time.
                        spawn_lookahead_worker(ProcID, Epoch, TargetSlot + 1, Opts)
                end,
...
%% @doc Schedule a fully-loaded message after resolving its process ID.
do_post_schedule(Base, Req, ToSched, Opts) ->
    ...
    case hb_message:with_only_committed(ToSched, Opts) of
        {ok, OnlyCommitted} ->
            ...
            case maybe_accept_remote_transfer(ProcID, Base, OnlyCommitted, Opts) of
                {ok, Assignment} ->
                    {ok, Assignment};
                {error, Error} ->
                    {error, Error};
                false ->
                    % Find the relevant scheduler server for the given process
                    % and message, start a new one if necessary, or return a
                    % redirect to the correct remote scheduler.
...
%% @doc Normalize a scheduler-server result into a device response.
schedule_result(PID, Req) ->
...
%% @doc Accept a remote terminal marker if this node is the next scheduler.
maybe_accept_remote_transfer(ProcID, Base, Assignment, Opts) ->
    case hb_ao:get(<<"type">>, Assignment, not_found, Opts) of
        <<"Assignment">> ->
            case transfer_next_scheduler(Assignment, Opts) of
                false ->
                    false;
                {error, Error} ->
                    {error, Error};
                {transfer, NextScheduler} ->
                    Proc = find_process_message(ProcID, Base, Assignment, Opts),
                    case
                        valid_remote_transfer(
                            ProcID,
                            Proc,
                            Assignment,
                            NextScheduler,
                    ...
                    of
                        true ->
                            case stop_scheduler(ProcID, Opts) of
                                ok ->
                                    ok =
                                        cache_transfer_chain(
                                            ProcID,
                                            Proc,
                                            Assignment,
                                            Opts
                                        ),
                                    dev_scheduler_registry:find(ProcID, Proc, Opts),
                                    {ok, Assignment};
                                {error, Error} ->
                                    {error, Error}
                            end;
                        false ->
                            {error,
                                #{
                                    <<"status">> => 403,
                                    <<"body">> =>
                                        <<"Invalid Scheduler-Transfer handoff.">>
                                }
                            }
                    end
            end;
...
%% @doc Cache a validated transfer marker and valid embedded proof markers.
cache_transfer_chain(ProcID, Proc, Assignment, Opts) ->
    Epoch = hb_util:int(hb_ao:get(<<"epoch">>, Assignment, <<"0">>, Opts)),
    case hb_ao:get(<<"previous-transfer">>, Assignment, not_found, Opts) of
        not_found ->
            dev_scheduler_cache:write(Assignment, Opts);
        Previous ->
            case valid_previous_transfer(ProcID, Proc, Previous, Epoch, Opts) of
                true -> ok = cache_transfer_chain(ProcID, Proc, Previous, Opts);
                false -> ok
            end,
            dev_scheduler_cache:write(Assignment, Opts)
    end.

%% @doc Stop any existing local scheduler before accepting a remote handoff.
stop_scheduler(ProcID, Opts) ->
            ...
            after 1000 ->
                ...
                {error,
                    #{
                        <<"status">> => 503,
                        <<"body">> =>
                            <<"Scheduler-Transfer local scheduler did not stop.">>
                    }
                }
            end;
        _ ->
...
%% @doc Verify a remote terminal marker before accepting a handoff.
valid_remote_transfer(ProcID, Proc, Assignment, NextScheduler, Opts) ->
    local_scheduler_available(NextScheduler, Opts)
        andalso hb_message:verify(Assignment, signers, Opts)
        andalso transfer_process_matches(ProcID, Assignment, Opts)
        andalso transfer_scheduler_signed(ProcID, Proc, Assignment, Opts)
        andalso transfer_length_matches(Assignment, Opts).

%% @doc Return true if a terminal marker belongs to the requested process.
transfer_process_matches(ProcID, Assignment, Opts) ->
    case hb_ao:get(<<"process">>, Assignment, not_found, Opts) of
        not_found -> false;
        AssignmentProcID -> hb_util:id(AssignmentProcID) == hb_util:id(ProcID)
    end.

%% @doc Return true if the expected scheduler signed the terminal marker.
transfer_scheduler_signed(ProcID, Proc, Assignment, Opts) ->
    Signers = hb_message:signers(Assignment, Opts),
    Expected = expected_transfer_schedulers(ProcID, Proc, Assignment, Opts),
    lists:any(
        fun(Scheduler) ->
            Identity = scheduler_identity(Scheduler),
            lists:member(Identity, Expected) andalso lists:member(Identity, Signers)
        end,
        parse_schedulers(hb_ao:get(<<"scheduler">>, Assignment, [], Opts))
    ).

%% @doc Return schedulers that are allowed to publish a terminal marker.
expected_transfer_schedulers(ProcID, Proc, Assignment, Opts) ->
    case hb_util:safe_int(hb_ao:get(<<"epoch">>, Assignment, <<"0">>, Opts)) of
        {ok, 0} ->
            scheduler_identities(hb_ao:get_first(
                [
                    {Proc, <<"scheduler">>},
        ...
        {ok, Epoch} when Epoch < 0 ->
            [];
        {ok, Epoch} ->
            case previous_epoch_scheduler(ProcID, Proc, Epoch, Opts) of
                [] ->
                    previous_transfer_scheduler(ProcID, Proc, Assignment, Epoch, Opts);
                Schedulers ->
                    Schedulers
            end;
        _ ->
...
%% @doc Return normalized scheduler identities.
scheduler_identities(Schedulers) ->
    lists:map(fun scheduler_identity/1, parse_schedulers(Schedulers)).

%% @doc Return the scheduler that a cached previous epoch transferred to.
previous_epoch_scheduler(ProcID, Proc, Epoch, Opts) ->
    case dev_scheduler_cache:latest(ProcID, Epoch - 1, Opts) of
        not_found ->
            [];
        {Slot, _Base} ->
            cached_transfer_scheduler(ProcID, Proc, Epoch - 1, Slot, Opts)
    end.

%% @doc Return the scheduler proven by the embedded previous transfer marker.
previous_transfer_scheduler(ProcID, Proc, Assignment, Epoch, Opts) ->
    case hb_ao:get(<<"previous-transfer">>, Assignment, not_found, Opts) of
        not_found ->
            [];
        Previous ->
            transfer_scheduler_identities(ProcID, Proc, Previous, Epoch, Opts)
    end.

%% @doc Return the scheduler identities proven by a terminal marker.
transfer_scheduler_identities(ProcID, Proc, Assignment, Epoch, Opts) ->
    case valid_previous_transfer(ProcID, Proc, Assignment, Epoch, Opts) of
        false ->
            [];
        true ->
            case transfer_next_scheduler(Assignment, Opts) of
                {transfer, NextScheduler} ->
                    scheduler_identities(NextScheduler);
                _ ->
                    []
            end
    end.

%% @doc Verify a proof marker for the epoch before a remote handoff.
valid_previous_transfer(ProcID, Proc, Previous, Epoch, Opts) ->
    case hb_util:safe_int(hb_ao:get(<<"epoch">>, Previous, <<"-1">>, Opts)) of
        {ok, PreviousEpoch} ->
            PreviousEpoch == Epoch - 1
                andalso hb_message:verify(Previous, signers, Opts)
                andalso transfer_process_matches(ProcID, Previous, Opts)
                andalso transfer_scheduler_signed(ProcID, Proc, Previous, Opts)
                andalso transfer_length_matches(Previous, Opts)
                andalso
                    case transfer_next_scheduler(Previous, Opts) of
                        {transfer, _NextScheduler} -> true;
                        _ -> false
                    end;
        {error, _} ->
            false
...
%% @doc Return scheduler identities for the latest cached epoch.
cached_epoch_schedulers(ProcID, Proc, Opts) ->
    case dev_scheduler_cache:latest_epoch(ProcID, Opts) of
        {Epoch, Slot, _Base} ->
            case cached_transfer_scheduler(ProcID, Proc, Epoch, Slot, Opts) of
                [] when Epoch > 0 ->
                    previous_epoch_scheduler(ProcID, Proc, Epoch, Opts);
                Schedulers ->
                    Schedulers
...
%% @doc Return the next scheduler from a valid cached terminal marker.
cached_transfer_scheduler(ProcID, Proc, Epoch, Slot, Opts) ->
    case dev_scheduler_cache:read(ProcID, Epoch, Slot, Opts) of
        {ok, Assignment} ->
            transfer_scheduler_identities(ProcID, Proc, Assignment, Epoch + 1, Opts);
        not_found ->
            []
    end.
...
%% @doc Return true if this node has a local identity for a scheduler.
local_scheduler_available(Schedulers, Opts) ->
    lists:any(
        fun(Scheduler) ->
            Identity = scheduler_identity(Scheduler),
            case hb_opts:as(Identity, Opts) of
                {ok, SchedulerOpts} ->
                    case hb_opts:get(priv_wallet, not_found, SchedulerOpts) of
                        not_found -> false;
                        Wallet -> hb_util:human_id(Wallet) == Identity
                    end;
                {error, _} -> false
            end
        end,
        parse_schedulers(Schedulers)
    ).
...
find_server(ProcID, Base, ToSched, Opts) ->
    find_server(ProcID, Base, ToSched, true, Opts).

%% @doc Locate a scheduler without redirecting transferred historical reads.
find_server_read(ProcID, Base, Epoch, Opts) ->
    find_server(ProcID, Base, undefined, {read, Epoch}, Opts).

%% @doc Locate the correct scheduling server for a given process.
find_server(ProcID, Base, ToSched, FollowTransfer, Opts) ->
        ...
        not_found ->
            ...
            case dev_scheduler_registry:find(ProcID, false, Opts) of
                PID when is_pid(PID) ->
                    ...
                    local_server(ProcID, PID, FollowTransfer, Opts);
                not_found ->
...
%% @doc Return a local scheduler unless it has already transferred away.
local_server(ProcID, PID, true, Opts) ->
    case dev_scheduler_server:info(PID) of
        #{ transferred_to := NextScheduler } ->
            find_remote_scheduler(ProcID, parse_schedulers(NextScheduler), Opts);
        _ ->
            {local, PID}
    end;
local_server(ProcID, PID, {read, Epoch}, Opts) ->
    case dev_scheduler_server:info(PID) of
        #{ transferred_to := NextScheduler, epoch := TransferEpoch }
                when Epoch >= TransferEpoch ->
            find_remote_scheduler(ProcID, parse_schedulers(NextScheduler), Opts);
        _ ->
            {local, PID}
    end;
local_server(_ProcID, PID, false, _Opts) ->
    {local, PID}.
...
is_local_scheduler(ProcID, ProcMsg, Scheduler, Opts) ->
    case local_scheduler_available(Scheduler, Opts) of
        true ->
            {
                ok,
...
get_schedule(Base, Req, Opts) ->
    ...
    Epoch =
        case hb_ao:get(<<"epoch">>, Req, not_found, Opts) of
            not_found -> 0;
            EpochRes -> hb_util:int(EpochRes)
        end,
    ...
    case find_server_read(ProcID, Base, Epoch, Opts) of
        {local, _PID} ->
            generate_local_schedule(Format, ProcID, Epoch, From, To, Opts);
        {redirect, Redirect} ->
            ?event({redirect_received, {redirect, Redirect}}),
            case hb_opts:get(scheduler_follow_redirects, true, Opts) of
                true ->
                    case get_remote_schedule(ProcID, Epoch, From, To, Redirect, Opts) of
                        {ok, Res} ->
...
get_remote_schedule(RawProcID, Epoch, From, To, Redirect, Opts) ->
    ...
    {FromLocalCache, _} = get_local_assignments(ProcID, Epoch, From, To, Opts),
    ?event(debug_sched,
        {from_local_cache,
            {from, From},
            {to, To},
...
do_get_remote_schedule(ProcID, _Epoch, LocalAssignments, From, To, _, Opts)
    ...
    Res;
do_get_remote_schedule(ProcID, Epoch, LocalAssignments, From, To, Redirect, Opts) ->
    ...
    Path =
        case Variant of
            <<"ao.N.1">> ->
                <<
                    ProcID/binary,
                    "/schedule?epoch=", (integer_to_binary(Epoch))/binary,
                    FromBin/binary,
                    ToParam/binary
                >>;
            <<"ao.TN.1">> ->
...
%% @doc Forward a schedule request to a remote scheduler variant.
...
%% @doc Post a terminal transfer assignment to the next remote scheduler.
post_transfer(ProcID, NextScheduler, Assignment, Opts) ->
    case find_remote_scheduler(ProcID, parse_schedulers(NextScheduler), Opts) of
        {redirect, Redirect} ->
            post_remote_schedule(ProcID, Redirect, Assignment, Opts);
        {error, Error} ->
            {error, Error}
    end.
...
generate_local_schedule(Format, ProcID, Epoch, From, To, Opts) ->
    ...
    {Assignments, More} = get_local_assignments(ProcID, Epoch, From, To, Opts),
...
get_local_assignments(ProcID, Epoch, From, undefined, Opts) ->
    case dev_scheduler_cache:latest(ProcID, Epoch, Opts) of
        not_found ->
            % No assignments in cache.
            {[], false};
        {Slot, _} ->
            get_local_assignments(ProcID, Epoch, From, Slot, Opts)
    end;
get_local_assignments(ProcID, Epoch, From, RequestedTo, Opts) ->
    ...
    {
        read_local_assignments(ProcID, Epoch, From, ComputedTo, Opts),
        ComputedTo < RequestedTo
    }.

%% @doc Get the assignments for a process.
read_local_assignments(_ProcID, _Epoch, From, To, _Opts) when From > To ->
    [];
read_local_assignments(ProcID, Epoch, CurrentSlot, To, Opts) ->
    case dev_scheduler_cache:read(ProcID, Epoch, CurrentSlot, Opts) of
        not_found ->
...
%% @doc Test that terminal markers must carry a transfer-shaped body.
terminal_marker_requires_transfer_body_test_parallel() ->
    NextScheduler = hb_util:human_id(crypto:strong_rand_bytes(32)),
    OtherScheduler = hb_util:human_id(crypto:strong_rand_bytes(32)),
    ?assertMatch(
        {error, #{ <<"status">> := 500 }},
        validate_next_slot(
            #{},
...
%% @doc Test that scheduler ownership requires a matching local wallet.
local_scheduler_requires_matching_wallet_test_parallel() ->
    start(),
    ...
    Opts =
        #{
            ...
            <<"identities">> =>
                #{ Scheduler => #{ <<"priv-wallet">> => OtherWallet } }
        },
    Proc =
        hb_message:commit(
            test_process(Scheduler),
            #{ <<"priv-wallet">> => SchedulerWallet }
        ),
    ?assertEqual(
        false,
        is_local_scheduler(hb_message:id(Proc, all, Opts), Proc, Scheduler, Opts)
    ).

%% @doc Test that transferred read routing follows only new epochs.
transferred_read_routing_test_parallel() ->
    ...
    InfoLoop =
        fun Loop() ->
            receive
                {info, Reply} ->
                    Reply ! {info, #{ transferred_to => Scheduler, epoch => 1 }},
                    Loop()
            end
        end,
    PID = spawn(InfoLoop),
    ?assertEqual({local, PID}, local_server(ProcID, PID, {read, 0}, #{})),
    ?assertMatch(
        {redirect, #{ <<"location">> := <<"http://example.com">> }},
        local_server(ProcID, PID, {read, 1}, #{})
    ).
