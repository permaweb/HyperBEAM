...
request(_Base, Req, Opts) ->
    case hb_maps:find(<<"body">>, Req, Opts) of
        {ok, [RawProcess | Rest]} ->
            case load_process(RawProcess, Opts) of
                {ok, Process} -> request_process(Process, Rest, Req, Opts);
                _ -> {ok, Req}
            end;
        _ ->
            {ok, Req}
...
%% @doc Resolve recognized carrier processes and propagate their failures.
request_process(Process, Rest, Req, Opts) ->
    case hb_maps:get(<<"execution-device">>, Process, undefined, Opts) of
        <<"carrier@1.0">> ->
            case
                hb_ao:resolve(
                    Process,
            ...
            of
                {ok, State} ->
                    {ok,
                        Req#{
                            <<"body">> =>
                                [value_base(value(State, Opts), Opts) | Rest]
                        }
                    };
                Error ->
                    Error
...
%% @doc Fully load a process before inspecting its execution device.
load_process(Process, Opts) when ?IS_ID(Process) ->
    case hb_cache:read(Process, Opts) of
        {ok, Loaded} -> load_process(Loaded, Opts);
        _ -> not_found
    end;
load_process(Process, Opts) when ?IS_LINK(Process); is_map(Process) ->
    try {ok, hb_cache:ensure_all_loaded(Process, Opts)}
    catch
        _:_ -> not_found
    end;
load_process(_Process, _Opts) ->
    not_found.
...
compute(Base, Assignment, Opts) ->
    maybe
        {ok, Sold} ?= swap(seed(Base, Opts), Assignment, Opts),
        ...
        case tx_field(Body, <<"target">>, <<>>, Opts) of
            ...
            _ ->
                % A sparse assignment may arrive through `Assign-To'.
                {ok, Sold}
        end
    end.
...
%%% process id must be a real 43-character address so the transaction codec can
%%% carry it as the layer-1 target.
...
seeded_value_test() ->
    ...
    ?assertEqual(
        ...
        hb_maps:get(<<"target">>, value(Alive, Opts), not_found, Opts)
    ),
    ...
    ?assertEqual(
        not_found,
        hb_maps:get(<<"target">>, value(Reset, Opts), not_found, Opts)
    ).
...
request_dereferences_process_test() ->
    Store = hb_test_utils:test_store(hb_store_lmdb, <<"carrier-request">>),
    ok = hb_store:start(Store),
    Opts = #{ <<"store">> => [Store] },
    try
        Target = #{ <<"greeting">> => <<"hello">> },
        ...
        {ok, ProcessID} = hb_cache:write(Process, Opts),
        Req =
            #{
                <<"path">> => <<"request">>,
                <<"request">> => #{},
        ...
        BrokenProcess =
            Process#{
                <<"swap-device">> => <<"missing@1.0">>,
                <<"test-random-seed">> => crypto:strong_rand_bytes(16)
            },
        {ok, BrokenProcessID} = hb_cache:write(BrokenProcess, Opts),
        ?assertThrow(
            {error,
                {
                    device_not_loadable,
