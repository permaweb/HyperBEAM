...
write(ProcID, Slot, Msg, RawOpts) ->
    ...
    maybe
        ...
        {ok, _} ?= hb_cache:read(SlotNumPath, Opts),
        ok ?= hb_cache:link(Root, MsgIDPath, Opts),
        {ok, _} ?= hb_cache:read(MsgIDPath, Opts),
...
refresh(ProcID, Slot, RawOpts) ->
    Opts = lib_process:scoped_opts(RawOpts),
    CachedSlot = latest_slot(ProcID, Opts),
    case CachedSlot of
        {ok, LatestSlot} when Slot < LatestSlot ->
            ok;
        _ ->
            write_refreshed_at(ProcID, Slot, clock(Opts), Opts)
    end.

%% @doc Persist and read back a content-addressed freshness marker.
write_refreshed_at(ProcID, Slot, Timestamp, Opts) ->
    Marker = #{ <<"slot">> => Slot, <<"timestamp">> => Timestamp },
    MarkerPath = refreshed_path(ProcID),
    maybe
        {ok, Root} ?= hb_cache:write(Marker, Opts),
        ok ?= hb_cache:link(Root, MarkerPath, Opts),
        {ok, WrittenMarker} ?= hb_cache:read(MarkerPath, Opts),
        {ok, Slot, Timestamp} ?= parse_refreshed_at(WrittenMarker, Opts),
        ok
    end.
...
read_refreshed_at(ProcID, Opts) ->
    case hb_cache:read(refreshed_path(ProcID), Opts) of
        {ok, RefreshedAt} -> parse_refreshed_at(RefreshedAt, Opts);
        _ -> undefined
    end.

parse_refreshed_at(RefreshedAt, Opts) ->
    try
        {
            ok,
            hb_util:int(hb_maps:get(<<"slot">>, RefreshedAt, Opts)),
            hb_util:int(hb_maps:get(<<"timestamp">>, RefreshedAt, Opts))
        }
    catch _:_ ->
        undefined
    end.
...
latest_slot(ProcID, Opts) ->
    case read_refreshed_at(ProcID, Opts) of
        {ok, Slot, _} ->
            case hb_cache:read(path(ProcID, Slot, Opts), Opts) of
                {ok, _} -> {ok, Slot};
                _ -> latest_readable_slot(ProcID, Opts)
            end;
        _ ->
            latest_readable_slot(ProcID, Opts)
    end.

latest_readable_slot(ProcID, Opts) ->
    first_readable_slot(
        ProcID,
        lists:reverse(lists:sort(slots(ProcID, Opts))),
        Opts
    ).

first_readable_slot(_ProcID, [], _Opts) ->
    {error, not_found};
first_readable_slot(ProcID, [Slot | Rest], Opts) ->
    case hb_cache:read(path(ProcID, Slot, Opts), Opts) of
        {ok, _} -> {ok, Slot};
        _ -> first_readable_slot(ProcID, Rest, Opts)
    end.
...
process_cache_suite_test_() ->
    hb_store:generate_test_suite(
        ...
        [hb_test_utils:test_store(hb_store_lmdb, <<"process-cache">>)]
    ).
...
freshness_max_age(Opts) ->
    ...
    Marker = hb_cache:ensure_all_loaded(RawMarker, lib_process:scoped_opts(Opts)),
    ?assertEqual(Slot, maps:get(<<"slot">>, Marker)),
    ?assertEqual(100, maps:get(<<"timestamp">>, Marker)),
    % A marker only identifies a latest slot when the output is readable.
    ok = refresh(ProcID, Slot + 1, Opts#{ <<"process-clock">> => 150 }),
    ?assertMatch({ok, Slot, _}, latest(ProcID, Opts)),
    ok = refresh(ProcID, Slot, Opts#{ <<"process-clock">> => 100 }),
