...
%%%
%%% A process using this device is sequenced by `~arweave-scheduler@1.0'.
%%% Control transactions target the process directly. A payment between two
%%% addresses names the process in its `Assign-To' tag, so it also becomes a
%%% slot that the process can settle against. Its process message reads:
%%% <pre>
...
%%%       winston, tagged with the `order-id' and with `Assign-To' naming the
%%%       process.</li>
%%% </ul>
%%%
...
%%% `block-height' that scheduler assignments carry. That is the only clock the
...
%%% forever. A quiet process advances lazily on its next relevant transaction.
-module(dev_arweave_swap).
-implements(<<"arweave-swap@1.0">>).
%%% AO-Core API functions:
-export([info/0, compute/3, set/3]).
-export([init/3, snapshot/3, normalize/3]).
...
%%
%% Scheduler assignments explicitly route to `compute', but the device also
%% remains safe if a caller selects another key. So there is no `exports' list:
%% a key outside it would fall through to `~message@1.0', answer `not_found',
%% fail its slot and wedge the process permanently. There is no `excludes' list
%% either, since an excluded `set' or `keys' would let `~message@1.0' replace the
%% process's own state. `set' is therefore implemented here.
%%
...
%% @doc Lifecycle reads preserve deterministic swap state verbatim.
...
%%
%% Advance the clock, then compare the real transaction `target' against the
%% process. A direct target is a control message; any other scheduled target may
%% be a payment declaring this process in `Assign-To'. A transaction that is
%% neither leaves the state unchanged apart from the clock.
%% The process is identified from the assignment rather than from the process
%% message, which `lib_process:process_id/3' would re-verify on every slot.
compute(Base, Assignment, Opts) ->
...
%% `order-id' and declares this process in `Assign-To'. It counts as payment for
%% an order when it is addressed to that order's `recipient' and carries at
%% least the asking winston.
%%
...
%%
%% The common path is one integer comparison against the next height at which
%% anything happens, and only crossing it walks the orders.
...
%%% The tests resolve synthetic assignments through the public device path. The
%%% device reads chain data but performs no I/O, so a whole trade can be played
%%% out without a weave.
...
order_numbers_survive_the_cache_test() ->
    Store = hb_test_utils:test_store(hb_store_lmdb, <<"swap-order">>),
    ok = hb_store:start(Store),
    Opts = (test_opts())#{ <<"store">> => [Store] },
    try
        Written = #{ <<"orders">> => #{ <<"o">> => #{
            <<"order-id">> => <<"o">>,
            <<"quantity">> => 10,
        ...
        Order =
            hb_maps:get(
                <<"o">>,
                hb_maps:get(<<"orders">>, Loaded, #{}, Opts),
                #{},
                Opts
            ),
...
%%
%% Any address may target or assign a transaction to this process, so the
%% `device' key is the author's to choose. Reading plainly dispatches on it,
%% and a device answers only for absent keys -- so the attack is to omit one.
%% Here
...
%% @doc Sequence a relevant transaction into the process at a block height,
%% as `~arweave-scheduler@1.0' does.
...
next_relevant(Base, Height, Opts) ->
    apply_tx(
        Base,
        tx(
            ar_wallet:new(),
            #{ <<"target">> => ?PROCESS, <<"action">> => <<"nothing">> }
        ),
        Height,
        Opts
    ).
...
%% @doc A reservation lapses lazily when the next relevant transaction carries
%% the process beyond its deadline.
reservation_lapses_test() ->
    ...
    Lapsed = next_relevant(Reserved, 110 + 20 + 1, Opts),
...
%% @doc A transaction assigned to this process but naming no live order moves
...
assigned_non_payment_test() ->
    ...
    ?assertEqual(150, state(<<"swap-height">>, Ticked, 0, Opts)),
    ?assertEqual(<<"open">>, maps:get(<<"status">>, only_order(Ticked, Opts))),
    ?assertEqual(90, balance(Ticked, SellerAddr, Opts)).

%% @doc The scheduler routes assignments to `compute', but selecting another
%% device key directly must still apply the transaction rather than fail.
...
supply_is_conserved_test() ->
    ...
    Lapsed =
        next_relevant(
            apply_tx(Before, offer(Seller, 10, 500, 5, 20), 100, Opts),
            300,
            Opts
        ),
...
lapse_forfeits_collateral_test() ->
    ...
    ?assertEqual(0, balance(Reserved, BuyerAddr, Opts)),
    Lapsed = next_relevant(Reserved, 190 + 20 + 1, Opts),
...
%% @doc The next height at which anything happens is recorded, so assignments
%% before it cost a single comparison rather than a walk of the orders.
next_deadline_tracked_test() ->
    ...
    ?assertEqual(0, state(<<"next-deadline">>, Opened, 0, Opts)),
    OrderID = maps:get(<<"order-id">>, only_order(Opened, Opts)),
    Reserved =
        apply_tx(Opened, order_action(Buyer, <<"register-interest">>, OrderID), 110, Opts),
    ...
    ?assertEqual(
        110 + 20 + 1,
        state(<<"next-deadline">>, Reserved, 0, Opts)
    ).

%% @doc Process snapshot and restore lifecycle calls do not advance or rewind
%% the swap clock and its reservation state.
lifecycle_preserves_swap_state_test() ->
    Opts = test_opts(),
    State =
        #{
            <<"device">> => <<"process@1.0">>,
    ...
    {ok, Snapshot} =
        hb_ao:resolve(State, #{ <<"path">> => <<"snapshot">> }, Opts),
    {ok, Restored} =
        hb_ao:resolve(
            Snapshot#{ <<"device">> => <<"arweave-swap@1.0">> },
            #{ <<"path">> => <<"normalize">> },
            Opts
        ),
    ?assertEqual(123, state(<<"swap-height">>, Restored, -1, Opts)),
    ?assertEqual(140, state(<<"next-deadline">>, Restored, -1, Opts)),
    ?assertEqual(
        <<"reserved">>,
        state(
            [<<"orders">>, <<"order">>, <<"status">>],
