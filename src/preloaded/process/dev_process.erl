...
%% of the given key.
-spec as(map(), map(), map()) -> {ok, #{ device := binary(), _ => _ }}.
as(RawBase, Req, Opts) ->
...
-spec compute(map(), map(), map()) -> {ok, #{ _ => _ }} | {error, _} | {failure, _}.
...
target_slot(Req, _Opts) ->
    case maps:get(<<"compute">>, Req, maps:get(<<"slot">>, Req, not_found)) of
        not_found -> not_found;
        Slot -> hb_util:int(Slot)
    end.
