...
-spec get_cookie(#{ _ => _ }, #{ key := binary(), format => binary(), _ => _ }, map()) -> term().
...
from(Msg, Req, Opts) ->
    CookieOpts = opts(Opts),
    LoadedMsg = load_cookie_fields(Msg, Opts),
