...
maybe_index_ids(Block, headers, Opts) ->
    TXIDs = hb_maps:get(<<"txs">>, Block, [], Opts),
    case fetch_block_tx_layout(Block, Opts) of
        error ->
            {block_skipped, #{
                skipped_count => length(TXIDs),
                total_txs => length(TXIDs)
            }};
        {ok, Layout} when length(Layout) =:= length(TXIDs) ->
            Results = parallel_map(
                Layout,
                fun(Entry) -> cache_data_free_header(Entry, Opts) end,
                Opts
            ),
            {block_cached,
...
cache_data_free_header({_TXID, DataSize}, _Opts)
        when is_integer(DataSize), DataSize > 0 ->
    counters(0, 0, 0);
cache_data_free_header({TXID, DataSize}, Opts) ->
    EncodedTXID = hb_util:encode(TXID),
    case resolve_tx_header(EncodedTXID, Opts) of
        {ok, #tx{ data_size = 0 } = TX} ->
            try
...
process_pending_tx(_TXID, headers, _Opts) ->
    counters(0, 0, 0);
...
%% @doc Read transaction IDs and data sizes from one `/block2' response.
fetch_block_tx_layout(Block, Opts) ->
    Height = hb_maps:get(<<"height">>, Block, 0, Opts),
        ...
        hb_http:request(
            #{
                ...
                <<"body">> =>
                    binary:copy(
                        <<255>>,
                        (length(hb_maps:get(<<"txs">>, Block, [], Opts)) + 7)
                            div 8
                    ),
                <<"route-by">> => Height
            },
...
parse_block2_transactions(
    ...
    parse_block2_tags(Rest);
parse_block2_transactions(_Bin) ->
    error.

parse_block2_tags(<< Count:16, Rest/binary >>) when Count =< 2048 ->
    parse_block2_tags(Count, Rest);
parse_block2_tags(_Bin) ->
    error.

parse_block2_tags(0, Rest) ->
    parse_block2_txs(Rest);
parse_block2_tags(Count, << Size:16, _:Size/binary, Rest/binary >>) ->
    parse_block2_tags(Count - 1, Rest);
parse_block2_tags(_Count, _Bin) ->
    error.

parse_block2_txs(<< Count:16, Rest/binary >>) when Count =< 1000 ->
    parse_block2_txs(Count, Rest, []);
parse_block2_txs(_Bin) ->
    error.

parse_block2_txs(0, _Rest, TXs) ->
    {ok, TXs};
parse_block2_txs(
    ...
    case parse_block2_tx(TXBin) of
        {ok, TX} ->
            parse_block2_txs(Count - 1, Rest, [TX | TXs]);
        error ->
            error
    end;
parse_block2_txs(_Count, _Bin, _TXs) ->
    error.
...
parse_block2_tx(
    <<
        ...
        AnchorSize:8, _:AnchorSize/binary,
        OwnerSize:16, _:OwnerSize/binary,
        TargetSize:8, _:TargetSize/binary,
        QuantitySize:8, _:(QuantitySize * 8),
        DataSizeSize:16, DataSize:(DataSizeSize * 8),
        DataRootSize:8, _:DataRootSize/binary,
        SignatureSize:16, _:SignatureSize/binary,
        RewardSize:8, _:(RewardSize * 8),
        DataEncodingSize:24, _:DataEncodingSize/binary,
        _/binary
    >>
...
headers_mode_test() ->
    ...
    ?assertMatch({ok, _}, hb_cache:read(ZeroTXID, LocalOpts)),
