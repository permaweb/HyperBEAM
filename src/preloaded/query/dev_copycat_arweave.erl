...
%% @doc Cache the data-free header of every plain L1 transaction in the block,
%% and index nothing else: no weave offsets, and no walk into bundles.
%%
%% This is the cheapest mode there is, and it exists for consumers that need
%% only to read each transaction back -- an `all'-mode schedule, where every
%% transaction in the block is a message. The headers are resolved together,
%% across `arweave_index_workers', where a consumer left to itself would fetch
%% them one at a time as it walked the block.
maybe_index_ids(Block, headers, Opts) ->
    TXIDs = hb_maps:get(<<"txs">>, Block, [], Opts),
    case resolve_tx_headers(TXIDs, Opts) of
        error ->
            {block_skipped,
                #{ skipped_count => length(TXIDs), total_txs => length(TXIDs) }};
        {ok, TXs} ->
            Results = parallel_map(TXs, fun(TX) -> cache_header(TX, Opts) end, Opts),
            {block_cached,
...
%% @doc Cache the data-free header of a plain layer-1 transaction. A bundle's
%% header is deliberately not cached: it would shadow the full bundle that the
%% bundle indexer writes, and the modes that do not walk bundles have nothing to
%% shadow it with. Consumers that sequence bundles too -- an `all'-mode schedule
%% does -- fetch their headers on demand instead.
cache_header(TX, Opts) ->
    case is_bundle_tx(TX, Opts) of
        false ->
            ok = cache_tx_header(TX, Opts),
