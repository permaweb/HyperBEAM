...
update(Reference, ShouldBlockPath, RetryAfter, State = #{ blocked := Blocked }, Now) ->
    case account_balance(Reference, State, Now) of
        ...
        _ ->
            case {ShouldBlockPath, maps:get(Reference, Blocked, 0)} of
                {true, _} ->
                    Until = Now + (RetryAfter * 1000),
                    {{blocked, RetryAfter},
                        State#{ blocked => Blocked#{ Reference => Until } }};
                {false, Until} when Until > Now ->
...
block_path_resets_expiry_test() ->
    ...
    {{blocked, 2}, UnchangedState} =
        update(caller, false, 3, ResetState, 2_000),
    ?assertEqual(ResetState, UnchangedState).
