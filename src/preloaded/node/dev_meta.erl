        ...
        EncOperator ->
            ...
            ?event(
                {is_operator,
                    ...
                    {verified_committers, VerifiedCommitters}
                }
            ),
            lists:member(EncOperator, VerifiedCommitters)
    end;
    ...
    case hb_ao:get(<<"method">>, Request, NodeMsg) of
        <<"POST">> ->
            case is_permanent(NodeMsg) of
                ...
                false ->
                    % AO routing has narrowed the path to the resolved function,
                    % so restore the committed path before signature checks.
                    update_node_message(
