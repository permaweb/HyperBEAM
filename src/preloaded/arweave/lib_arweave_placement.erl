...
%% @doc Write one placement, its alias and its byte offset when an Arweave index
%% store is configured, and return the link the block carries in its place.
written(Placement, Store, Opts) ->
    maybe
        ...
        ok ?=
            hb_cache:link(
                MsgID,
                lib_arweave_paths:placement(ID),
                Opts
            ),
        {ok, _Written} ?=
