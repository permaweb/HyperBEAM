...
%% trusted checkpoint. See `lib_arweave_sync'.
%%
...
bootstrap(Base, Req, Opts) ->
    lib_arweave_sync:bootstrap(Base, Req, Opts).
...
validated(Base, Req, Opts) ->
    lib_arweave_sync:validated(Base, Req, Opts).

%% @doc Return where a source transaction, named by `tx', occurs in the chain
%% this node validated. See `lib_arweave_sync'.
placement(Base, Req, Opts) ->
    lib_arweave_sync:placement(Base, Req, Opts).
...
block({id, ID}, Req, Opts) ->
    ...
    % hash is -- so both ask `lib_arweave_sync' rather than each keeping a
    % check that can drift from the other.
    maybe
        {ok, ID} ?= lib_arweave_sync:block_hash(ID),
...
%% lives.
block_by_id(ID, Req, Opts) ->
    case hb_cache:read(lib_arweave_cache:hash_path(ID), Opts) of
        {ok, Block} ->
...
block_by_height(Height, Req, Opts) ->
    case lib_arweave_cache:read(Height, Opts) of
        {ok, Block} ->
            ?event(arweave_short, {read_block_from_cache,
                {height, Height}
            }),
...
to_message(Path = <<"/block/", _/binary>>, <<"GET">>, {ok, #{ <<"body">> := Body }}, LogExtra, Opts) ->
    ...
    CacheRes =
        case hb_opts:get(arweave_index_blocks, true, Opts) of
            true -> lib_arweave_cache:write(Block, Opts);
            false -> skipped
        end,
    ?event(
        debug_arweave_index,
