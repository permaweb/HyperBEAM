...
%% @doc Extending a materialized head reads no historical links. Resetting the
%% store makes any such read fail, while the public push still extends the
%% window the head carries privately.
materialized_window_avoids_link_reads_test() ->
    Opts = test_opts(),
    Values = intervals(100),
    Head = block_time_history(Values, Opts),
    ok = hb_store:reset(stores(Opts)),
    {ok, Extended} =
        hb_ao:resolve(
            Head,
            #{
...
%%
%% What the list holds is the cap exactly, in order, and a walk asking for more
%% than the cap ends at it rather than reaching an entry beyond it.
trims_at_the_cap_test() ->
    ...
    Head =
        lib_arweave_history:from_values(
            <<"block-time-history">>, Values, 1275480, Opts
        ),
...
%% @doc A history read through one store cannot borrow entries from another.
%%
%% The head is written and read back, so what is asked for is the message the
%% cache produced rather than the original term. The second store holds that
%% head alone, so materialising the history through it must fail on the first
%% missing link even after the same process materialised it through the first.
store_isolation_survives_a_cache_round_trip_test() ->
    Opts = test_opts(),
    Head = block_time_history(intervals(5), Opts),
    {ok, ID} = hb_cache:write(Head, Opts),
    {ok, Reread} = hb_cache:read(ID, Opts),
    ?assertEqual(intervals(5), lib_arweave_history:values(Reread, Opts)),
    Partial = test_opts(),
    {ok, _} = hb_cache:write(Head, Partial),
    ?assertThrow(
        {necessary_message_not_found, _, _},
        lib_arweave_history:values(Reread, Partial)
    ).
...
%% @doc A history built from consensus values is extended through the public
%% device over generic AO-Core messages.
%%
%% Rendering the persistent result into Arweave's binary representation proves
%% the AO-Core state denotes the same consensus values.
push_and_to_binary_round_trip_test() ->
    ...
    Head = lib_arweave_history:from_values(
        <<"block-time-history">>, intervals(4), 1275480, Opts),
...
%% @doc The same public push operation holds for the reward history, whose
%% entries carry four fields of their own rather than three.
%%
...
reward_history_push_round_trip_test() ->
    ...
    Head = lib_arweave_history:from_values(
        <<"reward-history">>, Values, 1500000, Opts),
...
%% @doc Render a history through the application-layer peer codec.
to_binary(Head, Opts) ->
    Values = lib_arweave_history:values(Head, Opts),
    case hb_maps:get(<<"kind">>, Head, <<>>, Opts) of
        <<"reward-history">> ->
