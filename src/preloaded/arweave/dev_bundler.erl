...
reconstitute_tx(TXRequest, Proofs) ->
    ...
    ChunksWithOffsets = lists:map(
        fun(ChunkRequest) ->
            ...
            Proof = hb_maps:flatten(ProofJSON, #{}),
            Offset = hb_util:int(maps:get(<<"offset">>, Proof)),
            Chunk = hb_util:decode(maps:get(<<"chunk">>, Proof)),
            DataRoot = hb_util:decode(maps:get(<<"data_root">>, Proof)),
            DataSize = hb_util:int(maps:get(<<"data_size">>, Proof)),
            DataPath = hb_util:decode(maps:get(<<"data_path">>, Proof)),
