...
%%% whole of the index: the message holds the header and the components the
%%% checks produced as explicit AO-Core links. The block device resolves the
%%% scalar `previous-block'
%%% through that same index, so the chain is reachable as
%%% `tip/previous/previous'. `~arweave@2.9/tip' points at the
...
%%%
%%% A block's presence under its hash is its publication marker, and its
%%% `validation/checks' and transaction counts say which backfill profile that
%%% publication completes. A header publication completes `headers'; an
%%% `archive' or `full' request also requires the transaction check and one
%%% placement per carried transaction. Publication writes transactions,
%%% placements and configured byte offsets first; verifies already-persisted
%%% components; and links the hash last. Nothing else records progress, and a
%%% partial pass leaves nothing to undo.
-module(lib_arweave_sync).
...
%%% that the block device -- which resolves `previous-block' -- and this one,
%%% which publishes the named block, cannot disagree about the store layout.
...
%% @doc Decode each peer page in the application layer, then append generic AO
%% entry messages. `start-height' makes a lost or reordered page fail rather
%% than splicing a hole into the weave.
ingest_index([], Index, _Entries, _Opts) ->
    {ok, Index};
ingest_index([{ok, Page, _Peer} | Pages], Index, Entries, Opts) ->
    maybe
        {ok, Decoded} ?= decode_index_page(Page),
        {ok, Extended} ?=
            hb_ao:resolve(
...
%% @doc Convert `/block_index2' transport bytes to ordered AO entry messages.
decode_index_page(Page) ->
    case ar_serialize:binary_to_block_index(Page) of
        {ok, NewestFirst} ->
            {ok,
                hb_util:list_to_numbered_message(
                    ...
                    || {Hash, WeaveSize, TXRoot} <- lists:reverse(NewestFirst)
                    ]
                )
            };
        {error, invalid_input} ->
            {error,
                error_message(
                    ...
                    <<"A peer returned a malformed block index page.">>
                )
            }
    end.
...
%% them into the persistent form a chain state carries belongs to this
%% application layer. The history device receives only generic entry messages.
%%
...
%% @doc Turn a verified peer document into the persistent list a chain state
%% carries.
%%
%% The height goes with the bytes because it bounds the history's length, and
%% the checkpoint's height is the one the state that holds it extends from.
stored_history(Kind, Body, Height, Opts) ->
    lib_arweave_sync_codec:decode_history(Kind, Body, Height, Opts).
        ...
        {ok, Anchor} ->
            ...
            maybe
                ...
                ok ?=
                    hb_cache:link(
                        ID,
                        lib_arweave_paths:accounts_anchor(),
                        Opts
                    ),
                {ok, _} ?=
                    hb_cache:read(lib_arweave_paths:accounts_anchor(), Opts),
                ok
            end;
        _ ->
...
%% @doc Decode `/wallet_list/<root>[/<cursor>]' pages in the application layer
%% and fold their generic account messages through `~arweave-wallets@2.9'.
%%
%% The accumulated state is threaded back in as the base of the next call
%% because it carries the incomplete Patricia tree privately. Only the final,
%% root-checked tree is durable.
%%
...
wallet_list(_Peers, Root, <<"last">>, Accounts, _Left, Opts) ->
    hb_ao:resolve(
        Accounts#{ <<"device">> => <<"arweave-wallets@2.9">> },
        #{ <<"path">> => <<"finalize">>, <<"expected-root">> => Root },
        Opts
    );
...
wallet_list(Peers, Root, Cursor, Accounts, Left, Opts) ->
    maybe
        ...
        {ok, NextCursor, Wallets} ?= wallet_page(Body),
        {ok, Next} ?=
            hb_ao:resolve(
                Accounts#{ <<"device">> => <<"arweave-wallets@2.9">> },
                #{
                    <<"path">> => <<"insert">>,
                    <<"accounts">> => account_messages(Wallets)
                },
                Opts
            ),
        wallet_list(
...
%% @doc Parse and validate one peer wallet-list document.
wallet_page(Body) ->
    case lib_arweave_sync_codec:decode_wallet_page(Body) of
        {ok, _Cursor, _Wallets} = Parsed -> Parsed;
        {error, Message} ->
            {error,
                error_message(
                    Message,
                    <<"The wallet-list page is not a valid peer document.">>
                )}
    end.

%% @doc Convert peer account tuples to the generic message the device accepts.
account_messages(Wallets) ->
    maps:from_list(
        [
            {
        ...
        || {Address, Account} <- Wallets
        ]
    ).

%% @doc Convert the peer cursor to the URL representation the next request uses.
wallet_cursor(last) -> <<"last">>;
wallet_cursor(Cursor) -> hb_util:encode(Cursor).
...
apply_blocks(_Peers, [], Applied, _Carried, _Opts) ->
    {ok, Applied};
apply_blocks(Peers, [Block | Blocks], Applied, Carried, Opts) ->
    Hash = hb_maps:get(<<"indep-hash">>, Block, [], Opts),
    Started = erlang:monotonic_time(millisecond),
    case apply_block(Peers, Block, Hash, Carried, Opts) of
        {ok, Next} ->
            report_applied(Block, Hash, Next, Started, Opts),
            apply_blocks(Peers, Blocks, Applied + 1, {Hash, Next}, Opts);
        Error ->
            report_rejected(Block, Hash, Error, Started, Opts),
            Error
    end.

apply_block(Peers, Block, Hash, Carried, Opts) ->
    maybe
        {ok, Base} ?=
            base(
                Carried,
...
%% @doc The block a block is applied onto -- the base of the resolve below:
%% the one this pass has just produced, when that is its parent, and the
%% published block read back from the store otherwise.
%%
%% The two are the same message. `write_placements/2' replaces the block's
%% placements with links to the very messages it has just written, so the stored
%% form differs from the one still in hand only in holding those placements by
%% link rather than inline. Every reader of that key resolves it through
%% `hb_maps:get/4' and none matches on it, which is what makes the two
%% interchangeable rather than merely similar.
base({Hash, Carried}, Hash, _Opts) ->
    {ok, Carried};
base(_Carried, Previous, Opts) ->
    hb_cache:read(block_key(Previous), Opts).
...
transaction(Peers, ID, Opts) ->
    case
        first_peer_decoded(
            ...
            <<"/tx/", ID/binary>>,
            fun(Body) ->
                lib_arweave_sync_codec:decode_transaction(Body, Opts)
            end,
            Opts
        )
    of
...
announce(Hash, Block, Opts) ->
    maybe
        ...
        ok ?= hb_cache:link(block_key(Hash), settled_path(Hash), Opts),
        ok
    end.
...
%% should hold, a block complete for the requested profile is skipped, and a
%% pass that stops early is repeated by re-issuing the same request. Recording
%% how far the backfill had got would be a second kind of progress state beside
%% the blocks themselves, and the brief has exactly one.
...
%% block is complete for the requested profile costs a cache read and nothing
%% else, which is what makes re-issuing the same request the way to resume.
...
%% @doc Materialise the block the index records at a height, unless the stored
%% block is complete for the requested profile.
materialize_at(Peers, Index, Height, Profile, Opts) ->
    maybe
        ...
        case hb_cache:read(block_key(Hash), Opts) of
            {ok, Block} ->
                case complete_for_profile(Profile, Block, Opts) of
                    true -> {ok, present};
                    false ->
                        materialize_new(
...
%% @doc Whether a published block carries everything a profile requires.
%% The validation marker is checked before either ordered transaction list is
%% loaded.
complete_for_profile(<<"headers">>, _Block, _Opts) ->
    true;
complete_for_profile(Profile, Block, Opts)
        when Profile == <<"archive">>; Profile == <<"full">> ->
    lists:member(<<"transactions">>, checks(Block, Opts))
        andalso length(
            hb_util:message_to_ordered_list(
                hb_maps:get(<<"transactions">>, Block, [], Opts),
                Opts
...
complete_for_profile(_Profile, _Block, _Opts) ->
    false.
...
%% record: it is linked under its own hash only once every required index
%% derived from it is written, so a block that reads back is a block whose
%% publication work is finished. That ordering is the whole of the guarantee,
%% and it is why the steps are not interchangeable:
%%
%% <ol>
%% <li>the transaction messages, each of which the generic match index picks up
%%     as it is written, so a query can find them;</li>
%% <li>the placements and, when an Arweave index store is configured, their byte
%%     offsets, which link the transactions written in step one;</li>
...
publish(Block, Hash, TXs, Opts) ->
    maybe
        ...
        {ok, Linked} ?= link_components(Placed, Opts),
        {ok, ID} ?=
            hb_cache:write(
                Linked,
                Opts#{ <<"match-index">> => false }
            ),
        ok ?= hb_cache:link(ID, block_key(Hash), Opts),
        {ok, _Published} ?= hb_cache:read(block_key(Hash), Opts),
        {ok, ID}
    end.

%% @doc Replace already-persisted carried components with their AO-Core links
%% before the containing block is written. The component devices own those
%% graphs; publishing a block links their roots rather than recursively writing
%% them as if they were new block fields.
link_components(Block, Opts) ->
    link_components(
        [
            <<"accounts">>,
            <<"block-index">>,
...
link_components([], Block, _Opts) ->
    {ok, Block};
link_components([Key | Keys], Block, Opts) ->
    case maps:get(Key, Block, []) of
        Component when is_map(Component) ->
            Stored = hb_private:reset(Component),
            ID = hb_message:id(
                Stored,
                none,
                ...
                _ ->
                    {error,
                        error_message(
                            ...
                            <<"A carried consensus component was not durable "
                                "before block publication.">>
                        )}
            end;
        _AbsentOrLinked ->
            link_components(Keys, Block, Opts)
    end.

%% @doc Build the eager content link a containing block carries.
component_link(ID) ->
...
peer_block(Peers, Hash, Opts) when is_binary(Hash) ->
    case
        first_peer_decoded(
            Peers,
            <<"/block2/hash/", Hash/binary>>,
            fun(Body) -> lib_arweave_sync_codec:decode_block(Body, Opts) end,
            Opts
        )
    of
        {ok, Block, _Peer} -> {ok, Block};
        Error -> Error
    end;
...
%% @doc Fetch and decode a path from the first peer that serves valid bytes.
%% A successful HTTP response with malformed content is a failure of that peer,
%% not evidence that the consensus object named by the path is invalid.
first_peer_decoded([], _Path, _Decoder, _Opts) ->
    {error, error_message(<<"no-peer-decoded">>,
        <<"No peer served a valid response for the requested path.">>)};
first_peer_decoded([Peer | Peers], Path, Decoder, Opts) ->
    case body(peer_get(Peer, Path, Opts)) of
        {ok, Body} ->
            case Decoder(Body) of
                {ok, Decoded} -> {ok, Decoded, Peer};
                _ -> first_peer_decoded(Peers, Path, Decoder, Opts)
            end;
        _ ->
            first_peer_decoded(Peers, Path, Decoder, Opts)
    end.
...
%% @doc Run `Fun' as the store's only consensus mutation at a time.
%%
...
%%
%% `hb_name' is BEAM-global, so the name carries the identity `hb_store' uses
%% for each configured store instance. Different nodes sharing a BEAM remain
%% independent unless they deliberately share the same store.
exclusive(_Task, Fun, Opts) ->
    Runner = hb_name:singleton(name(Opts), fun runner/0),
...
%% @doc The registration name of the runner for a set of store instances.
name(Opts) ->
    {arweave_sync, store_refs(hb_opts:get(store, [], Opts))}.

%% @doc The stable identities `hb_store' assigns the configured stores.
