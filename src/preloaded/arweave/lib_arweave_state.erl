...
%%%
%%% The recent-block window is not carried at all. A block links its parent, so
%%% the window is the chain itself, read back one block at a time and stopped
%%% at the anchor depth. Duplicating it into every block would store the same
%%% fifty summaries fifty times over, and they are already in the blocks.
%%%
...
-export([previous_block/2, accounts/2]).
...
%%
%% The walk follows each block's `previous-block' hash rather than its
%% `previous' link, and stops at the first ancestor this node does not hold.
%% Both matter. A block may link a parent it has not downloaded -- every node
%% that joined from a checkpoint has one at the bottom of its chain -- and
%% loading such a link raises rather than answering `not_found', so a walk
%% through the links would crash there. Stopping instead is also what makes the
%% window correct: a block this node does not hold is not one a transaction may
%% anchor against.
