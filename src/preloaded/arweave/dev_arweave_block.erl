%%% @doc An AO-Core interface to Arweave's block rules: the three block hashing
%%% primitives and the state transition over canonical block messages.
%%%
...
%%%
%%% The checks cover Arweave's five validation stages, grouped under eleven
%%% stable names and run cheapest first rather than in stage order -- see
%%% `checks/0'. Difficulty has one authoritative check, while block field sizes
%%% are enforced structurally by `ar_serialize:binary_to_block/1'. Every proof
%%% of work, proof of access, VDF and RandomX check runs for every candidate;
%%% this device deliberately has no re-signed-solution shortcut.
%%%
...
%%% checks after the application layer has decoded the peer's wire response
%%% through `ar_serialize:binary_to_block/1', which enforces every field size
%%% structurally.
...
apply(Base, Req, Opts) ->
    maybe
        {ok, Selected} ?= lib_arweave_block:selected(Req, Opts),
        State = lib_arweave_state:materialize_histories(Base, Opts),
        {ok, Prev, Next, NextMsg, TXs} ?= inputs(State, Req, Selected, Opts),
        {ok, Ran, Accounts} ?= run(Selected, Prev, Next, TXs, State, Opts),
        ok ?= require_accounts(Accounts, Opts),
        transition(State, Prev, Next, NextMsg, Accounts, Ran, Opts)
    end.
...
validate(Base, Req, Opts) ->
    maybe
        {ok, Selected} ?= lib_arweave_block:selected(Req, Opts),
        State = lib_arweave_state:materialize_histories(Base, Opts),
        {ok, Prev, Next, _NextMsg, TXs} ?= inputs(State, Req, Selected, Opts),
        {ok, Ran, _Accounts} ?= run(Selected, Prev, Next, TXs, State, Opts),
...
materialize(Base, Req, Opts) ->
    maybe
        {ok, Selected} ?=
            lib_arweave_block:selected(Req, <<"archive">>, Opts),
...
%% @doc Read the validated block named by this block's parent hash.
%%
%% An Arweave block hash is not an AO-Core message identifier, so it remains a
%% scalar block field and is resolved through the consensus cache only when the
%% path is followed. This also lets a materialized block name an older block
%% before backfill has downloaded it.
previous(Base, _Req, Opts) ->
    Hash = hb_maps:get(<<"previous-block">>, Base, not_found, Opts),
    case is_binary(Hash) andalso
        ...
        _ ->
            {error,
                #{
                    ...
                    <<"detail">> =>
                        <<"The previous block is not in the validated cache.">>
                }
            }
    end.
...
run(Selected, Prev, Next, TXs, Block, Opts) ->
    maybe
        ...
        ok ?= when_wanted(<<"reward-history">>, Selected,
            fun() -> lib_arweave_block:check_reward_history_hash(Next, Prev) end),
            ...
            {error,
                error_message(400, <<"unavailable-check">>,
                    << "Materialising against a block index cannot establish ",
                        (hb_util:bin(lists:join(<<", ">>, Unavailable)))/binary,
...
check_fields(Next, Prev, Opts) ->
    maybe
        ...
        ok ?= lib_arweave_block:check_step_number(Next, Prev),
...
check_chunk_hashes(Next = #block{ recall_byte2 = undefined }) ->
    maybe
        ...
        lib_arweave_block:holds(Next#block.poa2 == #poa{}, <<"invalid-chunk-hash">>,
...
check_second_unpacked_chunk_hash(Next = #block{ recall_byte2 = undefined }) ->
    lib_arweave_block:holds(Next#block.unpacked_chunk2_hash == undefined,
        <<"invalid-unpacked-chunk-hash">>,
...
check_timestamp(Next, Prev) ->
    lib_arweave_block:holds(ar_block:verify_timestamp(Next, Prev), <<"invalid-timestamp">>,
...
check_last_retarget(Next, Prev) ->
    lib_arweave_block:holds(ar_block:verify_last_retarget(Next, Prev),
...
check_difficulty(Next, Prev) ->
    lib_arweave_block:holds(ar_retarget:validate_difficulty(Next, Prev),
        <<"invalid-difficulty">>,
...
check_cumulative_diff(Next, Prev) ->
    lib_arweave_block:holds(ar_block:verify_cumulative_diff(Next, Prev),
...
check_proof(RecallByte, Declared, PoA, Next, Prev, State, Message, Opts) ->
    maybe
        ...
        lib_arweave_block:holds(hb_maps:get(<<"valid">>, Result, false, Opts) == true, Message,
...
block_bounds(RecallByte, Prev, State, Opts) ->
    maybe
        ok ?= lib_arweave_block:holds(RecallByte < Prev#block.weave_size,
            <<"invalid-recall-byte">>,
...
check_vdf(Next, Prev, Opts) ->
    maybe
        ...
        lib_arweave_block:holds(hb_maps:get(<<"valid">>, Result, false, Opts) == true,
            <<"invalid-vdf-chain">>,
...
%% transactions.
check_weave_size(Next, Prev) ->
    lib_arweave_block:holds(ar_block:verify_weave_size(Next, Prev, Next#block.txs),
        <<"invalid-weave-size">>,
...
check_block_time_history(Next, Prev) ->
    maybe
        ok ?= lib_arweave_block:check_block_time_history_hash(Next, Prev),
...
check_txs(Next, Prev, Balances, Block, Opts) ->
    ...
    lib_arweave_block:holds(ar_tx_replay_pool:verify_block_txs(Args) == valid,
        <<"invalid-txs">>,
...
check_tx_root(Next) ->
    lib_arweave_block:holds(ar_block:verify_tx_root(Next), <<"invalid-tx-root">>,
...
check_block_index_root(Next, Prev) ->
    lib_arweave_block:holds(ar_block:verify_block_hash_list_merkle(Next, Prev),
...
%% @doc Extend the branch-local block index with the validated block, leaving
%% an absent index absent. Historical blocks already retain earlier roots, so
%% the component does not duplicate the block DAG as an index-version chain.
next_block_index(Block, Next, Opts) ->
    lib_arweave_block:extend_block_index(
        lib_arweave_state:block_index(Block, Opts), Next, Opts).
