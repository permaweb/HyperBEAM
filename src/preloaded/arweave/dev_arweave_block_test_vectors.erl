...
selected_checks_test() ->
    Names = [ Name || {Name, _Needs} <- dev_arweave_block:checks() ],
    ?assertEqual(
        {ok, Names},
        dev_arweave_block:selected(#{ <<"profile">> => <<"full">> }, #{})
    ),
    % No profile at all is full validation.
    ?assertEqual({ok, Names}, dev_arweave_block:selected(#{}, #{})),
    ?assertEqual(
        {ok, [<<"linkage">>, <<"identity">>]},
        dev_arweave_block:selected(
    ...
    ?assertMatch(
        ...
        dev_arweave_block:selected(#{ <<"verify">> => <<"identtiy">> }, #{})
    ),
    ?assertMatch(
        {error, #{ <<"message">> := <<"unknown-profile">> }},
        dev_arweave_block:selected(#{ <<"profile">> => <<"quick">> }, #{})
    ),
    ...
    ?assertMatch(
        ...
        dev_arweave_block:selected(#{ <<"verify">> => <<"accounts">> }, #{})
    ),
    ?assertMatch(
        {error, #{ <<"message">> := <<"incomplete-checks">> }},
        dev_arweave_block:selected(#{ <<"verify">> => <<"poa">> }, #{})
    ),
    ?assertMatch(
        {ok, _},
        dev_arweave_block:selected(
...
%% @doc A block-index state links the version it was derived from, and the link
%% resolves to that version.
%%
%% The version chain is attached where the previous version is known to have
%% been published -- which is here, because it was published as a component of
%% the block being extended. The index state is written and read back first, so
%% what the link is computed against is the message the cache produced rather
%% than the one that was handed to it: if those disagreed the link would point
%% at nothing, and a walk of the index's own history would end at its newest
%% version.
block_index_links_its_previous_version_test() ->
    Opts = test_opts(),
    {ok, Index} =
        hb_ao:resolve(
            ...
            #{
                <<"path">> => <<"append">>,
                <<"indep-hash">> => hb_util:encode(<<1:384>>),
                <<"weave-size">> => 262144,
                <<"tx-root">> => hb_util:encode(<<2:256>>)
            },
            Opts
        ),
    % Publish it as a component of a block, which is the only way one is
    % stored.
    {ok, ID} =
        hb_cache:write(
            #{
                <<"device">> => <<"arweave-block@2.9">>,
                <<"indep-hash">> => hb_util:encode(<<1:384>>),
                <<"block-index">> => hb_private:reset(Index)
            },
            Opts
        ),
    {ok, Block} = hb_cache:read(ID, Opts),
    {ok, Extended} =
        dev_arweave_block:extend_block_index(
            hb_maps:get(<<"block-index">>, Block, not_found, Opts),
            #block{
                indep_hash = <<3:384>>,
                weave_size = 524288,
                tx_root = <<4:256>>
    ...
    ?assertEqual(2, hb_util:int(hb_maps:get(<<"length">>, Extended, 0, Opts))),
    ?assertEqual(
        1,
        hb_util:int(
            hb_maps:get(
                <<"length">>,
                hb_maps:get(<<"previous">>, Extended, not_found, Opts),
                0,
                Opts
            )
        )
