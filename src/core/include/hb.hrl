...
%% @doc Macro usable in guards that validates whether a term is an ID.
-define(IS_ID(X),
    (is_binary(X) andalso
        (byte_size(X) == 42 orelse
            byte_size(X) == 43 orelse
            byte_size(X) == 32 orelse
            (byte_size(X) >= 6 andalso
                binary_part(X, 0, 6) =:= <<"ao-id:">>)))).
