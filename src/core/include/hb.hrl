...
%% human-readable ID: a 32/42/43-byte native/eth/base64url ID, or an
%% `ao:'-prefixed ID. The `byte_size > 3' guard keeps `binary_part/3' from
%% raising, so the macro is safe in guards and expressions alike.
-define(IS_ID(X), (is_binary(X) andalso (byte_size(X) == 42 orelse byte_size(X) == 43 orelse byte_size(X) == 32 orelse (byte_size(X) > 3 andalso binary_part(X, 0, 3) == <<"ao:">>)))).
