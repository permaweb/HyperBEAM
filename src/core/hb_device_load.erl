...
%%%
%%% <b>Forge build.</b> If `forge-bootstrap' is set in `Opts' the device
%%% search <em>is</em> a map lookup inside it -- nothing else. Only the
%%% Forge build sets it, supplying the seed codecs under their module
%%% names so it can compute IDs and sign the preloaded messages.
%%%
...
%%%       `forge-bootstrap' / `trusted-devices' maps; the build-signed
%%%       preloaded store.</li>
...
%% @doc A message or an atom is already a device. A binary reference is
%% resolved, then memoised in the process cache.
reference(Loaded, _Opts) when is_map(Loaded); is_atom(Loaded) ->
    {ok, Loaded};
reference(Ref, Opts) when is_binary(Ref) ->
    NormRef = hb_ao:normalize_key(Ref),
    case resolve(NormRef, Opts) of
        {ok, Mod} = Ok -> put_resolved_device(NormRef, Mod, Opts), Ok;
        {error, _} = Error -> Error
    end.
...
from_high_trust(Ref, Opts) ->
    maybe
        {error, not_found} ?= from_trusted_map(forge_bootstrap, Ref, Opts),
        {error, not_found} ?= from_trusted_map(trusted_devices, Ref, Opts),
        from_preloaded(Ref, Opts)
    end.

%% @doc A trusted map keys a device name or ID to either an
%% already-loaded module atom or an implementation archive ID.
from_trusted_map(Key, Ref, Opts) ->
    case hb_opts:get(Key, #{}, Opts) of
