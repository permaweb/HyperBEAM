...
%% VENDOR: upstream is `-include("ar.hrl")'. `rebar.config' sets
%% {i, "src/core"}, so the header resolves as include/ar.hrl here.
...
%% VENDOR: upstream is `b64fast:encode(Bin)'. HyperBEAM has no b64fast NIF;
%% its base64url is `hb_util' over the b64veryfast NIF. Proven byte-identical
%% to RFC 4648 s5 unpadded base64url - see VENDOR.md.
encode(Bin) ->
	hb_util:encode(Bin).

%% @doc Try to decode a URL safe base64 into a binary or throw an error when
%% invalid.
%% VENDOR: upstream is `b64fast:decode(Input)', which raises on invalid input.
%% `hb_util:decode/1' is the *unchecked* b64veryfast decoder: it returns
%% garbage instead of raising for some malformed inputs, which would silently
%% turn `safe_decode/1' below into a function that never reports `{error,
%% invalid}'. The checked decoder underneath `hb_util' is used instead - it
%% agrees with `hb_util:decode/1' on every valid base64url input and raises on
%% the rest, matching b64fast. `iolist_to_binary/1' stands in for the iolist
%% clause `hb_util:decode/1' has and the NIF does not; it is a no-op on a
%% binary.
decode(Input) ->
	b64veryfast:decode64_url(iolist_to_binary(Input)).
...
%% VENDOR: upstream's parse_peer_test/0 is dropped. It drives parse_peer/2
%% through `ar_test_inet_mock', a module of the upstream test harness that
%% HyperBEAM does not vendor. parse_peer/1,2 themselves are unchanged.
...
%% VENDOR: not an upstream test. `between/3' is the clamp under
%% `ar_difficulty:scale_diff/3' and `ar_pricing_transition:get_transition_price/2',
%% and the latter passes `infinity' as the upper bound, relying on Erlang's term
%% order (every number sorts below every atom) for the upper clamp to be a no-op.
%% Reversing either comparison in `between/3' fails this test.
between_test() ->
	%% The ordinary three-way clamp.
	?assertEqual(10, between(5, 10, 20)),
	?assertEqual(20, between(25, 10, 20)),
	?assertEqual(15, between(15, 10, 20)),
	?assertEqual(10, between(10, 10, 20)),
	?assertEqual(20, between(20, 10, 20)),
	%% The premise: every number sorts below every atom.
	?assert(0 < infinity),
	?assert((1 bsl 256) < infinity),
	?assertNot(0 > infinity),
	%% `infinity' as the upper bound means "no upper bound" - the value comes
	%% back unchanged, never the atom. A reversed upper comparison
	%% (N < Max -> Max) returns `infinity' for every one of these.
	lists:foreach(
		fun(N) ->
			?assertEqual(N, between(N, 0, infinity))
		end,
		[0, 1, 42, 1000000, 1 bsl 64, 1 bsl 256]
	),
	%% The lower clamp still bites under an `infinity' upper bound. A reversed
	%% lower comparison (N > Min -> Min) returns 5 for the first and 100 for
	%% the second.
	?assertEqual(100, between(5, 100, infinity)),
	?assertEqual(1000, between(1000, 100, infinity)),
	%% The exact shape `ar_pricing_transition:get_transition_price/2' relies on.
	?assertEqual(123456789, between(123456789, 0, infinity)).
