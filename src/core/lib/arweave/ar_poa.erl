...
%%% Should track: https://github.com/ArweaveTeam/arweave/blob/master/apps/arweave/src/ar_poa.erl
-module(ar_poa).

-export([get_data_path_validation_ruleset/2, get_data_path_validation_ruleset/3,
		 validate/1, chunk_proof/4, chunk_proof/5,
	...
	case has_redundant_rebase_marker_for_chunk(DataRoot, Offset, TXSize, DataPath, Chunk) of
		true ->
			log_invalid_data_path(redundant_rebase_marker, Peer,
					[{data_root, hb_util:encode(DataRoot)}, {offset, Offset}, {tx_size, TXSize}]),
...
%% VENDOR: upstream formats the peer with ar_util:format_peer/1. There is no
%% ar_util here, so the peer term is logged as-is.
...
%% VENDOR: `validate/1' and the `validate2'/`validate3' chain reach the
%% unpacker through `ar_packing_server:unpack/5' and `unpack_sub_chunk/5', which
%% take the RandomX state from a process-wide singleton. This port passes the
%% state explicitly, so the vendored `ar_packing_server' exports those functions
%% at arity 6 and these call sites cannot run. `~arweave-spora@2.9' implements
%% the same validation against the arity-6 forms and is what the block device
%% actually calls; this chain has no in-tree caller. Kept so the module diffs
...
%% VENDOR: upstream's chunk_proof/2 and chunk_proof/3 (src/ar_poa.erl:172-193)
%% resolve the block bounds by reading the ar_block_index ETS table. They serve
%% the local chunk-storage and chunk-verification paths, not block validation,
%% and the ETS table does not exist here; ar_block_index:get_block_bounds/2 is
%% the store-agnostic replacement. Only the block-validation entry points below
%% are vendored.
...
%% VENDOR: the two prometheus_counter:inc(validating_packed_spora, ...) calls
%% upstream makes in validate2/2 and validate3/2 are dropped - HyperBEAM does not
%% declare that counter.
...
%% VENDOR: upstream's validate_pre_fork_2_5/4 and its
%% validate_data_path_pre_fork_2_5/4 and validate_chunk_pre_fork_2_5/2 helpers
%% (src/ar_poa.erl:363-397) are dropped - this subsystem validates post-2.9
%% blocks only.
