		...
		fun(_Case) ->
			...
			lists:foldl(
				...
				fun(Permutation, Acc) ->
					...
					%% VENDOR: upstream returns Acc here, so the accumulator stays
					%% `start' forever and the permutation-invariance assertion above
					%% never fires. Returning H makes the test assert what its own
					%% comment says it asserts - the property the account tree
					%% bootstrap relies on, since a peer serves the accounts in
