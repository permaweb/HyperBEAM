...
%%% Should track:
%%% https://github.com/ArweaveTeam/arweave/blob/master/apps/arweave/src/ar_pricing.erl
%%%
%%% VENDOR: this module is vendored nearly whole. Both reward formulas are live on
%%% mainnet: v2 pricing does not activate until height 2,069,870, so every block we
%%% validate today still takes the 2.5-era get_miner_reward_and_endowment_pool/1 path,
%%% while price_per_gib_minute is already computed by the v2 code. Removed: the four
%%% prometheus gauges and the debug logging they sit next to (log_price_metrics/11 and
%%% network_data_size/5), the LOCALNET overrides of the redenomination knobs, and the
%%% tests.
%%%
%%% All post-2.5 arithmetic here is exact integer or exact {Dividend, Divisor} fraction
%%% arithmetic. The float branches are unreachable above fork 2.5 and are kept only so
%%% the diff against upstream stays mechanical.
-module(ar_pricing).
...
%% 2.5 exports.
%% VENDOR: dropped get_tx_fee/4 and get_storage_cost/4, and with them
...
%% ar_tx reaches the 2.6 get_tx_fee/1 instead - and get_storage_cost/4 is called once,
%% from ar_node_worker, to publish a price metric. Neither is on a validation path.
%% They are also the only users of ?TX_SIZE_BASE, which src/core/include/ar.hrl
%% defines, so restoring them is mechanical.
...
-export([get_v2_price_per_gib_minute/2, get_v2_price_per_gib_minute_two_difficulty/2]).

-include("include/ar.hrl").
-include("include/ar_inflation.hrl").
...
get_v2_price_per_gib_minute_one_difficulty(_Height, B) ->
	{HashRateTotal, RewardTotal, _History} = ar_rewards:get_reward_history_totals(B),
	{IntervalTotal, VDFIntervalTotal, OneChunkCount, TwoChunkCount} =
		ar_block_time_history:sum_history(B),
	%% The intent of the SolutionsPerPartitionPerVDFStep is to estimate network replica
...
%%
%% VENDOR: dropped the -ifdef(LOCALNET) clauses of get_redenomination_threshold/0 and
%% get_redenomination_delay_blocks/0, which read application env overrides.
