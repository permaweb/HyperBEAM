...
-export([base64_address_with_optional_checksum_to_decoded_address/1]).
...
%% @doc Decode a base64url address that may carry the optional ":<crc32>"
%% checksum suffix. Vendored from ArweaveTeam/arweave @ 50e47de,
%% src/ar_wallet.erl:331; `ar_serialize:json_struct_to_block/1' needs it.
%% VENDOR: `ar_util:decode/1' is HyperBEAM's `hb_util:decode/1'.
base64_address_with_optional_checksum_to_decoded_address(AddrBase64) ->
	Size = byte_size(AddrBase64),
	case Size > 7 of
		false ->
			hb_util:decode(AddrBase64);
		true ->
			case AddrBase64 of
				<< MainBase64url:(Size - 7)/binary, ":", ChecksumBase64url:6/binary >> ->
					AddrDecoded = hb_util:decode(MainBase64url),
					...
					Checksum = hb_util:decode(ChecksumBase64url),
