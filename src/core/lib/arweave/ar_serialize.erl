...
-export([block_to_binary/1, binary_to_block/1, json_struct_to_block/1,
		...
		nonce_limiter_info_to_json_struct/2, find_value/2]).

-include("include/ar.hrl").
-include("include/ar_consensus.hrl").
-include("include/ar_vdf.hrl").

%% VENDOR: the decoder callbacks that make OTP's `json' module produce jiffy's
%% eep18 term shape - an object becomes `{[{BinaryKey, Value}, ...]}' with the
...
block_to_json_struct(
	...
	JSONElements =
		[{nonce, hb_util:encode(Nonce2)}, {previous_block, hb_util:encode(PrevHash)},
				{timestamp, TimeStamp}, {last_retarget, LastRetarget}, {diff, JSONDiff},
				{height, Height}, {hash, hb_util:encode(Hash)},
				{indep_hash, hb_util:encode(IndepHash)},
				{txs,
					lists:map(
				...
				}, {tx_root, hb_util:encode(TXRoot)}, {tx_tree, []},
				{wallet_list, hb_util:encode(WalletList)},
				{reward_addr,
					case RewardAddr of unclaimed -> list_to_binary("unclaimed");
							_ -> hb_util:encode(RewardAddr) end}, {tags, Tags2},
				{reward_pool, JSONRewardPool}, {weave_size, JSONWeaveSize},
				{block_size, JSONBlockSize}, {cumulative_diff, JSONCDiff},
				{hash_list_merkle, hb_util:encode(MR)}, {poa, poa_to_json_struct(POA)}],
	JSONElements2 =
			...
			true ->
				...
				JSONElements6 =
					[{hash_preimage, hb_util:encode(B#block.hash_preimage)},
							...
							{signature, hb_util:encode(B#block.signature)},
							{reward_key, hb_util:encode(element(2, B#block.reward_key))},
							...
							{reward_history_hash,
									hb_util:encode(B#block.reward_history_hash)},
		...
		case Height >= ar_fork:height_2_7() of
			true ->
				JSONElements7 = [
						...
						{chunk_hash, hb_util:encode(B#block.chunk_hash)},
						{block_time_history_hash,
							hb_util:encode(B#block.block_time_history_hash)}
						| JSONElements5],
				case B#block.chunk2_hash of
					undefined ->
						JSONElements7;
					_ ->
						[{chunk2_hash, hb_util:encode(B#block.chunk2_hash)} | JSONElements7]
				end;
			false ->
				JSONElements5
		end,
		...
		case Height >= ar_fork:height_2_8() of
			...
			true ->
				case {PackingDifficulty >= 1, RecallByte2} of
					...
					{true, undefined} ->
						[{packing_difficulty, PackingDifficulty},
							{unpacked_chunk_hash, hb_util:encode(UnpackedChunkHash)}
							| JSONElements8];
					_ ->
						[{packing_difficulty, PackingDifficulty},
							{unpacked_chunk_hash, hb_util:encode(UnpackedChunkHash)},
							{unpacked_chunk2_hash, hb_util:encode(UnpackedChunk2Hash)}
							| JSONElements8]
				end
		end,
	JSONElements10 =
...
%% VENDOR: upstream uses jiffy. HyperBEAM has no jiffy dependency, so the
%% encoder is OTP 27+'s `json' module driven by a custom value encoder that
%% renders upstream's eep18 objects (`{Proplist}', with atom keys) exactly the
%% way jiffy does. `json:encode_key_value_list/2' emits atom keys as strings and
%% `{[]}' as `{}', matching jiffy byte for byte.
jsonify(JSONStruct) ->
	iolist_to_binary(json:encode(JSONStruct, fun encode_json_value/2)).
...
%% VENDOR: upstream's json_decode/2 takes jiffy options; there is no equivalent
%% here, so only the arity-1 form is vendored. The decoder callbacks reproduce
%% jiffy's default eep18 shape: an object becomes `{[{BinaryKey, Value}, ...]}'
%% with the keys in document order.
json_decode(JSON) ->
	try json:decode(JSON, ok, ?JSON_EEP18_DECODERS) of
		{DecodedJSON, ok, <<>>} ->
			{ok, DecodedJSON};
		{_DecodedJSON, ok, _Rest} ->
			{error, trailing_data}
	catch
		_:Reason ->
			{error, Reason}
...
%%
%% VENDOR: upstream asserts `Height < ar_fork:height_2_6()' (src/ar_serialize.erl:1357)
%% and there is no post-2.6 JSON parser anywhere in the upstream tree - post-2.6
%% blocks are only ever parsed from the `/block2' binary format by
%% binary_to_block/1. The assertion is preserved verbatim rather than
%% "fixed": inventing a post-2.6 JSON parser would be new consensus code, not a
%% port. Use binary_to_block/1 for post-2.6 blocks; block_to_json_struct/1
%% remains the fully fork-aware encoder in the other direction.
...
poa_to_json_struct(POA) ->
	Fields = [
		...
		{tx_path, hb_util:encode(POA#poa.tx_path)},
		{data_path, hb_util:encode(POA#poa.data_path)},
		{chunk, hb_util:encode(POA#poa.chunk)}
	],
	Fields2 =
		case POA#poa.unpacked_chunk of
			<<>> ->
				Fields;
			UnpackedChunk ->
				Fields ++ [{unpacked_chunk, hb_util:encode(UnpackedChunk)}]
		end,
	{Fields2}.

%% @doc Convert the nonce limiter info into a JSON struct.
%%
%% Note the three field renames applied here and nowhere else - the binary
%% encoder keeps the record's own names:
...
%% the per-step VDF outputs and `last_step_checkpoints' the 25 intra-step
%% checkpoints of the final step.
nonce_limiter_info_to_json_struct(Height,
	...
	Fields = [{output, hb_util:encode(Output)}, {global_step_number, N},
			{seed, hb_util:encode(Seed)},
			{next_seed, hb_util:encode(NextSeed)}, {zone_upper_bound, ZoneUpperBound},
			{next_zone_upper_bound, NextZoneUpperBound},
			{prev_output, hb_util:encode(PrevOutput)},
			{last_step_checkpoints, [hb_util:encode(Elem) || Elem <- Checkpoints]},
			%% Keeping  'checkpoints' as JSON key (rather than 'steps') for backwards
			%% compatibility.
			{checkpoints, [hb_util:encode(Elem) || Elem <- Steps]}],
	Fields2 =
		case Height >= ar_fork:height_2_7() of
			false ->
				Fields;
				...
				Keys3 =
					case TXRoot of
						not_set ->
							Keys2;
						_ ->
							[{<<"tx_root">>, hb_util:encode(TXRoot)} | Keys2]
					end,
				{Keys3};
			(BH) ->
				hb_util:encode(BH)
...
%% The eep18 object shape jiffy produces and upstream's find_value/2 expects.
encode_json_value({KVs}, Encode) when is_list(KVs) ->
	json:encode_key_value_list(KVs, Encode);
encode_json_value(Other, Encode) ->
