...
-export([get_consensus_window_size/0, get_max_tx_anchor_depth/0,
		...
		indep_hash/1, indep_hash2/2, get_block_signature_preimage/4,
		...
		get_block_bounds/2, get_block_bounds/3]).
...
%% VENDOR: see `compute_h0/5' -- this arity routes through it and so cannot run
%% either. `compute_h0/6' is the working entry point.
compute_h0(B, PrevB) ->
...
%% VENDOR: upstream reaches the RandomX state through a process-wide
%% `ar_packing_server:get_packing_state/0'. This port has no such singleton --
%% the state is owned by an `hb_name' process and passed explicitly -- so that
%% function is not vendored and these two arities cannot run. `compute_h0/6' is
%% the working entry point; `~arweave-spora@2.9' calls it with a state it holds.
%% They are kept rather than deleted so the module still diffs cleanly against
%% upstream, and made to fail with a named error rather than `undef'.
compute_h0(_NonceLimiterOutput, _PartitionNumber, _Seed, _MiningAddr,
		_PackingDifficulty) ->
	error(packing_state_not_threaded).
...
%%
%% VENDOR: upstream takes a CacheTab and reads the `ar_block_cache' and
%% `ar_block_index' ETS tables directly (src/ar_block.erl:749-776). HyperBEAM keeps
%% both the recent block cache and the block index in `hb_store'/`hb_cache', so the
%% three lookups upstream performs are supplied by the caller instead. `Index' is a
%% map, all keys optional:
%%   #{ oldest_block_start => non_neg_integer(),           % default 0
%%      get_block => fun((BH :: binary()) -> #block{} | not_found),
%%                                                          % default: not_found
%%      seek => ar_block_index:seek() }                     % default: not_found
%% The defaults degrade to "no cache, no index", which is what an empty map means.
get_block_bounds(RecallByte, PrevB) ->
	get_block_bounds(RecallByte, PrevB, #{}).

get_block_bounds(RecallByte, PrevB, Index) when RecallByte < PrevB#block.weave_size ->
	OldestStart = maps:get(oldest_block_start, Index, 0),
	case RecallByte < OldestStart of
		true ->
			ar_block_index:get_block_bounds(RecallByte, seek(Index));
		false ->
			get_block_bounds_from_cache(RecallByte, PrevB, Index)
	end;
get_block_bounds(_RecallByte, _PrevB, _Index) ->
	{error, invalid_recall_byte}.

get_block_bounds_from_cache(RecallByte, B, Index) ->
	...
	case RecallByte >= BlockStart of
		...
		false ->
			...
			case get_cached_block(PrevH, Index) of
				not_found ->
					ar_block_index:get_block_bounds(RecallByte, seek(Index));
				PrevB ->
					get_block_bounds_from_cache(RecallByte, PrevB, Index)
...
get_cached_block(BH, #{ get_block := GetBlock }) ->
	GetBlock(BH);
get_cached_block(_BH, _Index) ->
	not_found.

seek(#{ seek := Seek }) ->
	Seek;
seek(_Index) ->
	ar_block_index:empty().
