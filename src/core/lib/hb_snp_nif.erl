...
init() ->
    %% Graceful degrade (demo-enabling): without the dev_snp_nif .so the real
    %% SNP path is unavailable, but the module must still load so the node can
    %% serve the L0 (trustless) + L1 (node-signature) layers and the
    %% capability-gated mock. A genuinely broken real build is surfaced as a
    %% warning rather than silently swallowed.
    case ?load_nif_from_crate(dev_snp_nif, 0) of
        ok -> ok;
        {error, Reason} ->
            logger:warning(
                "dev_snp_nif NIF not loaded (~p); SNP real path disabled, "
                "running L0+L1 + capability-gated mock only",
                [Reason]
            ),
            ok
    end.
...
generate_attestation_report_test() ->
	...
	case hb_snp_nif:check_snp_support() of
		{ok, true} ->
			...
			?assertEqual(
				{ok, UniqueData},
				hb_snp_nif:generate_attestation_report(UniqueData, VMPL)
			);
		{ok, false} ->
...
compute_launch_digest_test() ->
	...
	?event(ArgsMap),
		...
		%% Call the NIF
	{ok, Result} = hb_snp_nif:compute_launch_digest(ArgsMap),
	%% Expected result
    EncTestVector =
...
verify_measurement_test() ->
	...
	%% Call the NIF
	Result = hb_snp_nif:verify_measurement(MockReport, ExpectedMeasurement),
