...
union_message_field_types_test() ->
    Integer = scalar_type(<<"integer">>),
    Binary = scalar_type(<<"binary">>),
    Union = #{ <<"kind">> => <<"union">>, <<"members">> =>
        [required(<<"value">>, Integer), required(<<"value">>, Binary)] },
    Message = #{ <<"value">> => <<"7">> },
    ?assertEqual(Message, apply_schema(Union, Message, #{})),
    Optional = message_type(
        #{ <<"value">> => #{ <<"presence">> => optional, <<"type">> => Integer } },
        none
    ),
    ?assert(check_type(Optional, #{}, #{})),
    ?assertNot(check_type(Optional, Message, #{})),
    TypedWildcard = message_type(
        #{}, #{ <<"presence">> => required, <<"type">> => Integer }
    ),
    ?assertNot(check_type(TypedWildcard, Message, #{})),
    ?assertEqual(
        Message,
        apply_schema(Union#{ <<"members">> =>
            [TypedWildcard, required(<<"value">>, Binary)] }, Message, #{})
    ).
...
type_alias_arities_test() ->
    TypeEnv = build_type_env([
        {attribute, 1, type, {value, {type, 1, integer, []}, []}},
        {attribute, 1, type,
            {value, {user_type, 1, value, []}, [{var, 1, 'Item'}]}}
    ]),
    Integer = scalar_type(<<"integer">>),
    ?assertEqual(
        Integer,
        parse_type({user_type, 1, value, []}, TypeEnv, #{}, [])
    ),
    ?assertEqual(
        Integer,
        parse_type(
            {user_type, 1, value, [{type, 1, binary, []}]},
            TypeEnv, #{}, []
        )
    ).
