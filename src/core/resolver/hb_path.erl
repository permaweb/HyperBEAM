%%% @doc Utilities for request paths and shared computation addresses.
%%%
%%% `hashpath/2' reads a message's private receipt, falling back to its ID.
%%% `hb_hashpath' formats, loads and verifies these transition claims.
%%%
%%% `hashpath/3' addresses a computation by its base and request IDs. The
%%% resolver uses this address for the varied execution shared by callers;
%%% it does not include the caller's preceding transition history.
%%% `hashpath/4' also supports folding a compact computation address with a
%%% further request, using the supplied hash algorithm.
-module(hb_path).
...
hashpath(Base, Req, Opts) when is_map(Base) ->
    BaseHashpath = message_id(Base, Opts),
...
hashpath(Base, Req, HashpathAlg, Opts) when is_map(Req) ->
    {ok, RequestPart} = hb_hashpath:format_request(Req, Opts),
    hashpath(Base, RequestPart, HashpathAlg, Opts);
...
%% @doc Calculate the signer-bound message ID without full AO-Core resolve.
message_id(Msg, Opts) ->
