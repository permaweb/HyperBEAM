...
read(Modules, Path, Opts) ->
    read(Modules, #{ <<"read">> => hb_path:concatenate_parts(Path) }, Opts).
...
group(Modules, Path, Opts) ->
    group(Modules, #{ <<"group">> => hb_path:concatenate_parts(Path) }, Opts).
...
type(Modules, Path, Opts) ->
    type(Modules, #{ <<"type">> => hb_path:concatenate_parts(Path) }, Opts).
...
resolve(Modules, Path, Opts) ->
    resolve(Modules, #{ <<"resolve">> => hb_path:concatenate_parts(Path) }, Opts).
...
list(Modules, Path, Opts) ->
    list(Modules, #{ <<"list">> => hb_path:concatenate_parts(Path) }, Opts).
