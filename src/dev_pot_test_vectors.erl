...
delegation_liquidation_test() ->
    ...
    ?assertEqual(1, dev_pot:get_deposit(Alice, ResourceOxygen, S3)),
    ?assertEqual(0, dev_pot:get_deposit(Bob, ResourceOxygen, S3)),
    ?assertEqual(0, dev_pot:get_deposit(Charlie, ResourceOxygen, S3)).
...
multiple_delegations_liquidation_test() ->
    ...
    report(S4),
    ?assertEqual(1, dev_pot:get_deposit(Alice, ResourceOxygen, S4)),
    ?assertEqual(1, dev_pot:get_deposit(Charlie, ResourceOxygen, S4)),
    S5 = dev_pot:undelegate(Alice, Bob, ResourceOxygen, 2, S4, Opts),
    ?assertEqual(2, dev_pot:get_deposit(Alice, ResourceOxygen, S5)),
    ?assertEqual(0, dev_pot:get_deposit(Bob, ResourceOxygen, S5)),
    ?assertEqual(0, dev_pot:get_deposit(Charlie, ResourceOxygen, S5)),
    ?assertEqual(0, dev_pot:get_deposit(Denis, ResourceOxygen, S5)).
...
cyclic_delegations_test() ->
    ...
    ?assertEqual(1, dev_pot:get_deposit(Alice, ResourceOxygen, S4)),
    ?assertEqual(0, dev_pot:get_deposit(Bob, ResourceOxygen, S4)),
    report(S4),
    S5 = dev_pot:undelegate(Bob, Alice, ResourceOxygen, 1, S4, Opts),
    report(S5),
    ?assertEqual(0, dev_pot:get_deposit(Alice, ResourceOxygen, S5)),
    ?assertEqual(1, dev_pot:get_deposit(Bob, ResourceOxygen, S5)).
...
deposit_removal_while_delegated_test() ->
    ...
    report(S1),
    ?assertEqual(1, dev_pot:get_deposit(Alice, ResourceOxygen, S3)),
    ?assertEqual(1, dev_pot:get_deposit(Bob, ResourceOxygen, S3)),
    ?assertEqual(1, dev_pot:get_deposit(Charlie, ResourceOxygen, S3)),
    S4 = dev_pot:withdraw(Alice, ResourceOxygen, 3, S3, Opts),
    report(S4),
    ?assertEqual(0, dev_pot:get_deposit(Alice, ResourceOxygen, S4)),
    ?assertEqual(0, dev_pot:get_deposit(Bob, ResourceOxygen, S4)),
    ?assertEqual(0, dev_pot:get_deposit(Charlie, ResourceOxygen, S4)).
...
drip_resource_with_zero_weight_test() ->
    ...
    S1 = dev_pot:drip(S0, #{<<"t">> => 1}, Opts),
    ?assertEqual(0, dev_pot:balance(Alice, S1)).
...
withdraw_exact_balance_test() ->
    ...
    ?assertEqual(0, dev_pot:get_deposit(Alice, ResourceOxygen, S1)).
...
delegate_to_self_test() ->
    ...
    ?assertEqual(10, dev_pot:get_deposit(Alice, ResourceOxygen, S1)),
...
delegate_entire_balance_test() ->
    ...
    ?assertEqual(0, dev_pot:get_deposit(Alice, ResourceOxygen, S1)),
    ?assertEqual(10, dev_pot:get_deposit(Bob, ResourceOxygen, S1)).
...
deep_delegation_chain_test() ->
    ...
    ?assertEqual(0, dev_pot:get_deposit(Bob, ResourceOxygen, S4)),
    ?assertEqual(0, dev_pot:get_deposit(Charlie, ResourceOxygen, S4)),
    ?assertEqual(0, dev_pot:get_deposit(Denis, ResourceOxygen, S4)),
    ?assertEqual(10, dev_pot:get_deposit(Eve, ResourceOxygen, S4)).
...
wide_delegation_tree_test() ->
    ...
    ?assertEqual(0, dev_pot:get_deposit(Alice, ResourceOxygen, S5)),
    ?assertEqual(2, dev_pot:get_deposit(Bob, ResourceOxygen, S5)),
    ?assertEqual(2, dev_pot:get_deposit(Charlie, ResourceOxygen, S5)),
    ?assertEqual(2, dev_pot:get_deposit(Denis, ResourceOxygen, S5)),
    ?assertEqual(2, dev_pot:get_deposit(Eve, ResourceOxygen, S5)),
    ?assertEqual(2, dev_pot:get_deposit(Frank, ResourceOxygen, S5)).
...
rapid_weight_changes_test() ->
    ...
    ?assertEqual(10, hb_maps:get(<<"total-weighted-units">>, S4, 0)).
...
drip_same_timestamp_idempotent_test() ->
    ...
    S0 = pot_state(Alice, ResourceOxygen, 10),
    S1 = dev_pot:drip(S0, #{<<"t">> => 1}, Opts),
    Minted1 = hb_maps:get(<<"minted">>, S1, 0),
    S2 = dev_pot:drip(S1, #{<<"t">> => 1}, Opts),
    Minted2 = hb_maps:get(<<"minted">>, S2, 0),
...
deposit_then_immediate_withdraw_test() ->
    ...
    S2 = dev_pot:withdraw(Alice, ResourceOxygen, 10, S1, Opts),
    ?assertEqual(0, dev_pot:get_deposit(Alice, ResourceOxygen, S2)),
    ?assertEqual(0, dev_pot:balance(Alice, S2)).
...
multiple_deposits_same_resource_test() ->
    ...
    ?assertEqual(10, dev_pot:get_deposit(Alice, ResourceOxygen, S3)).
...
multiple_delegations_outbox_order_test() ->
    ...
    ?assertEqual(Bob, hb_maps:get(<<"target">>, Notice1, Opts)),
    ?assertEqual(Charlie, hb_maps:get(<<"target">>, Notice2, Opts)).
...
deposit_to_multiple_resources_test() ->
    ...
    ?assertEqual(20, dev_pot:get_deposit(Alice, ResourceHydrogen, S2)),
    ?assertEqual(30, hb_maps:get(<<"total-weighted-units">>, S2, 0)).
...
resource_isolation_test() ->
    ...
    InitialOxygenDeposit = dev_pot:get_deposit(Alice, ResourceOxygen, S2),
    % Modify hydrogen
...
weighted_distribution_across_resources_test() ->
    ...
    AliceBalance = dev_pot:balance(Alice, S4),
    BobBalance = dev_pot:balance(Bob, S4),
...
very_large_deposit_test() ->
    ...
    ?assertEqual(LargeAmount, dev_pot:get_deposit(Alice, ResourceOxygen, S1)),
    ?assertEqual(LargeAmount, hb_maps:get(<<"total-weighted-units">>, S1, 0)).
