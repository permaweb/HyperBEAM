...
run(Args, NodeOpts) ->
    ...
    Groups =
        hb_packager:scan(default_preloaded_dirs(Dirs), #{}) ++
...
%% @doc Return the HyperBEAM preloaded source directory for external
%% repositories, unless the caller's source set already covers it.
default_preloaded_dirs(Dirs) ->
    case is_hb_checkout() andalso source_covers(<<"src/preloaded">>, Dirs) of
        true -> [];
        false -> find_default_preloaded_dir(Dirs)
    end.

%% @doc Find the first available HyperBEAM preloaded source directory.
find_default_preloaded_dir(Dirs) ->
    case [
        hb_util:bin(D)
     ||
        D <- [
            "_build/default/lib/hb/src/preloaded",
            "_build/default/plugins/plugin/src/preloaded",
            "src/preloaded"
        ],
        filelib:is_dir(D)
    ] of
        [] -> [];
        [Dir | _] ->
            case lists:any(fun(D) -> contains_dir(D, Dir) end, Dirs) of
                true -> [];
                false -> [Dir]
            end
    end.
