...
% http_post_process_now_test_() ->
%     { timeout, 900, fun http_post_process_now/0 }.
% http_post_process_now() ->
%     application:ensure_all_started(hb),
%
%     % Create AOS process using the same setup as spawn_and_execute_slot test
%     Msg1 = test_aos_process(),
%     ProcessID = hb_message:id(Msg1, all),
%     hb_cache:write(Msg1, #{}),
%
%     % Initialize the process
%     hb_ao:resolve(Msg1, Msg1#{<<"path">> => <<"schedule">>, <<"method">> => <<"POST">>}, #{}),
%
%     % Schedule the same computation as the working test
%     schedule_aos_call(Msg1, <<"return 1+1">>),
%     schedule_aos_call(Msg1, <<"return 2+2">>),
%
%     % Test the HTTP endpoint by making the same call through the existing HTTP server
%     % The default test process uses port 8734 as seen in the banner
%     Node = <<"http://localhost:8734">>,
%
%     % Create HTTP POST message
%     RequestMessage = hb_message:commit(#{
%         <<"path">> => <<"/", ProcessID/binary, "~process@1.0/now">>
%     }, hb:wallet()),
%
%     % Make HTTP POST request
%     {ok, Response} = hb_http:post(Node, RequestMessage, #{}),
%
%     % Verify response - it should be a message with results/data containing "4"
%     ?assertMatch(<<"4">>, hb_ao:get(<<"results/data">>, Response)).
-endif.