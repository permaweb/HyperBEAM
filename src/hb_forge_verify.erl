...
load_archive(Root, Archive) ->
    maybe
        {ok, Modules, Resources} ?= hb_device_archive:contents(Archive),
        ok ?= hb_device_archive:modules_match_root(Root, Modules),
        ImplDir = hb_ao_device:implementation_dir(Root),
        ok ?= hb_device_archive:write_resources(ImplDir, Resources),
