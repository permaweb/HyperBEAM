...
handle_mounting_partition(
    ...
    maybe
        ok ?= check_does_device_exist(Device),
        ok ?= maybe_create_and_format_partition(Device, Partition, PartitionType, Key),
        ok ?= mount_volume(Partition, Key, MountPoint, VolumeName),
        {ok, NewStore} ?= update_node_store(StorePath, Opts),
...
create_and_format_partition(Device, Partition, PartitionType, Key) ->
    ...
    case hb_volume:create_partition(Device, PartitionType) of
        ...
        {ok, _} ->
            ?event(debug_volume,
                {create_and_format_partition, partition_created, Partition}
            ),           
            case hb_volume:format_disk(Partition, Key) of
                {error, FormatError} -> 
                    ?event(debug_volume, 
                        {create_and_format_partition, formatting_error, 
                            {error, FormatError}
                        }
                    ),
                ...
                {ok, FormatResult} -> 
                    ?event(debug_volume, 
                        {create_and_format_partition, format_success, 
                            {result, FormatResult}
                        }
                    ),
                    ok  % Ready to mount
