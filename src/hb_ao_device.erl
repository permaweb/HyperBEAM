...
resolve_to_spec_id(Ref, Opts) ->
    resolve_name(Ref, Opts).

%% @doc Bootstrap the name device through the preloaded resolver message.
preloaded_name(Opts) ->
...
%% @doc Resolve non-bootstrap names through the name device.
resolve_name(Ref, Opts) ->
...
load_local_implementation(Ref, SpecID, Opts) ->
    case hb_cache:match(implementation_match_spec(SpecID), Opts) of
        {ok, IDs} -> load_implementation_ids(IDs, Ref, SpecID, Opts);
        _ -> not_found
    end.
...
load_archive(ModBin, Archive, Msg, Ref, Opts) ->
    case is_generated_module(ModBin) of
        ...
        true ->
            ...
            case archive_modules(Archive) of
                {ok, Modules} ->
                    do_load_archive(ModName, Modules, Msg, Opts);
                {error, Reason} ->
...
%% @doc Validate archive entries and gather them for `code:atomic_load/1'.
read_archive_modules([], Acc) ->
    Modules = [Mod || {Mod, _, _} <- Acc],
    case length(Modules) =:= length(lists:usort(Modules)) of
        true -> {ok, lists:reverse(Acc)};
        false -> {error, duplicate_archive_module}
    end;
read_archive_modules([{Path0, Beam} | Rest], Acc) ->
    Path = hb_util:bin(Path0),
    case archive_beam_module(Path, Beam) of
        {ok, Mod} ->
            read_archive_modules(Rest,
                [{Mod, binary_to_list(Path), Beam} | Acc]);
        {error, Reason} ->
            {error, Reason}
    end.
...
do_load_archive(RootMod, Modules, Msg, Opts) ->
    case lists:keymember(RootMod, 1, Modules) of
        ...
        true ->
            ...
            case archive_modules_match_root(RootBin, Modules) of
                ok -> maybe_load_archive(RootMod, Modules, Msg, Opts);
                {error, Reason} ->
                    {error, load_error(
...
archive_modules_match_root(RootBin, Modules) ->
    Bad =
        ...
        ||
            {Mod, _, _} <- Modules,
            not archive_module_matches_root(RootBin, atom_to_binary(Mod, utf8))
        ],
    case Bad of
        [] -> ok;
...
%% @doc Check one archive module against the root namespace.
archive_module_matches_root(RootBin, ModBin) ->
    Prefix = <<RootBin/binary, "__">>,
    ModBin =:= RootBin
        orelse binary:match(ModBin, Prefix) =:= {0, byte_size(Prefix)}.
    ...
    case archive_loaded(Modules) of
        ...
        false ->
            case code:atomic_load(Modules) of
                ...
                {error, Reason} ->
                    AlreadyLoaded =
                        all_not_purged(Reason) andalso archive_loaded(Modules),
...
%% @doc `code:atomic_load/1' may report `not_purged' for already-loaded code.
all_not_purged(Reason) when is_list(Reason) ->
    lists:all(fun({_Mod, not_purged}) -> true; (_) -> false end, Reason);
all_not_purged(_Reason) ->
    false.
            ...
            case hb_maps:get(<<"on-load-format">>, Msg, undefined, Opts) of
                ?ON_LOAD_FORMAT ->
                    case decode_archive_on_loads(OnLoad) of
                        {ok, OnLoads} ->
                            run_archive_on_load_list(OnLoads, Opts);
                        {error, _} = Error ->
                            Error
                    end;
                Other ->
...
run_archive_on_load_list([#{ <<"module-name">> := ModBin,
    ...
    case apply(Mod, Fun, []) of
        ok -> run_archive_on_load_list(Rest, Opts);
...
lookup_device_cache(Ref, Opts) ->
    case lookup_process_dict_device_cache(Ref) of
        {ok, _} = Ok -> Ok;
        not_found ->
...
%% @doc Look up the hot process-local device cache.
lookup_process_dict_device_cache(Ref) ->
    case erlang:get({?MODULE, device_cache, Ref}) of
        ModName when is_atom(ModName) -> loaded_module(ModName);
        _ -> not_found
    end.
