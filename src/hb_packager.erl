...
group_by_namespace(Files, Names, ForcedRoots) ->
    ...
    %% Resolve each file into a package root.
    %% If a file declares -implements(...), it is added to the roots list.
    %% If a file matches an existing root prefix, it is added to that root's helpers list.
    %% Otherwise, it is added to the roots list.
    {Roots, Helpers} =
        lists:foldl(
            fun({Module, _Path}, {RAcc, HAcc}) ->
                case sets:is_element(Module, ForcedRoots) of
                    ...
                    false ->
                        ...
                        case longest_root_prefix(Module, NameSet) of
                            Module ->
                                {[Module | RAcc], HAcc};
                            Root ->
                                {RAcc, [{Root, Module} | HAcc]}
                        end
                end
            end,
            {[], []},
    ...
    % For each root, create a package map, where:
    % root is the root module atom,
    % root_file is the binary path to the root module file,
    % helpers is a list of {helper module atom, helper file path binary} pairs,
    % files is a map of {file name binary, file content binary} pairs.
    [
        begin
            RootHelpers =
                lists:sort(
...
package(#{ root := Root, root_file := RootFile, helpers := Helpers, files := Files }, Opts) ->
    ...
    % If the root or any helpers try to load a NIF, reject.
    ...
    ArchivePkg = compile_archive(ModName, Root, RootFile, Helpers, SourceHash, Opts),
...
compile_archive(RootMod, Root, RootFile, Helpers, Hash, _Opts) ->
    ...
    Compiled =
        compile_renamed_modules(RenamedPaths, Copied, Renamings, OnLoads),
...
compile_renamed_modules(RenamedPaths, Copied, Renamings, OnLoads) ->
    [
        compile_renamed_module(Path, Copied, Renamings, OnLoads)
      ||
...
archive_module_metadata(Compiled) ->
    [
        #{
            ...
            <<"archive-path">> =>
                hb_util:bin(atom_to_list(Mod) ++ ".beam")
        }
      ||
        #{ module := Mod } <- Compiled
    ].

make_archive(Compiled) ->
    Files =
        [
            {
                atom_to_list(Mod) ++ ".beam",
                Beam,
                archive_file_info(byte_size(Beam))
            }
          ||
            #{ module := Mod, beam := Beam } <- Compiled
        ],
