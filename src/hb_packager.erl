...
%%% Source parsing. We use `epp_dodger' so we do not need include paths or macro
%%% definitions to read the file. The dodger output is a syntax-tree list (not
%%% necessarily reverted Erlang abstract forms): we keep it that way and pass it
%%% straight to Igor, which accepts syntax trees.
...
%% @doc Reject dynamic calls into the source package namespace.
...
%% @doc Find source forms that dynamically dispatch into internal modules.
...
compile_archive(
    ...
    CopiedPaths = [hb_util:list(Path) || #{ path := Path } <- Copied],
    OnLoads = package_on_loads(Renamings, Copied),
    IncludeDirs = include_dirs(Copied),
    % Igor rewrites the package into one generated namespace before compiling.
    IgorOpts = [
        {stubs, false},
        {backups, false},
        {comments, true},
        ...
        {tidy, false},
        {preprocess, true},
        {includes, [hb_util:list(Dir) || Dir <- IncludeDirs]}
    ],
...
%% @doc Build the old-module -> generated-module mapping passed to Igor.
...
%% @doc Return include dirs needed to compile renamed package sources.
include_dirs(Copied) ->
    lists:usort(
        [
            <<"src">>,
...
%% @doc Copy one source module into the isolated package workspace.
copy_package_source(TmpDir, Mod, Path) ->
    Target =
        filename:join(
            TmpDir,
            <<(atom_to_binary(Mod, utf8))/binary, ".erl">>
        ),
...
%% @doc Collect generated-module on_load callbacks for archive metadata.
...
%% @doc Return the copied source metadata for a module.
find_copied_module(Mod, Copied) ->
    [Found] = [C || C = #{ module := M } <- Copied, M =:= Mod],
    Found.

%% @doc Extract a pure Erlang `-on_load(Fun/0)' callback from attributes.
on_load_fun(Attrs) ->
...
%% @doc Compile every renamed source file into BEAM binaries.
compile_renamed_modules(
    RenamedPaths,
    Copied,
    Renamings,
...
%% @doc Compile one renamed module after applying test and on_load rewrites.
compile_renamed_module(Path, Copied, Renamings, OnLoads, IncludeDirs, Opts) ->
    ...
    % Persist `on_load' callbacks as archive metadata so runtime invokes them.
    ...
    CompileOpts =
        [
            binary,
            debug_info,
            {source, hb_util:list(filename_only(Source))}
        ] ++
...
%% @doc Restore HB's parallel-test parse transform after Igor preprocessing.
restore_test_parallel_transform(Path, Opts) ->
    case hb_maps:get(<<"test">>, Opts, false, Opts) of
...
%% @doc Insert the parallel-test parse transform in generated test sources.
...
%% @doc Find the original source path for a generated module.
...
%% @doc Return on_load functions that belong to a generated module.
...
%% @doc Remove `-on_load' attributes and export callbacks for archive loading.
strip_on_load(_Path, []) ->
    ok;
...
%% @doc Rewrite any source-level fun captures left after Igor renaming.
rewrite_surviving_captures(Path, Renamings) ->
    Source0 = read_file(Path),
    Source1 =
        lists:foldl(
...
%% @doc Rewrite captures for one old/new module name pair.
