...
# produced. Runs INSIDE the linux/amd64 builder container so the
# test exercises the same erts the guest will use at runtime,
# regardless of host architecture. Fails fast on host-vs-release
# OTP-major-version skew rather than leaving the guest to crash
# at boot with `error_loading'.
...
# The openssl BINARY is intentionally not shipped: dev_tpm2 reads the
# real EK certificate from TPM NV storage; nothing in the guest needs
# the userspace CLI. The shared libraries stay because HyperBEAM's
# crypto NIFs link against them.
