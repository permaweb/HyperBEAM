...
| 4 | Secure Boot / UKI signing pipeline is operator-provisioned at deploy time, not enforced at build | `scripts/sb-setup.sh` produces operator-owned PK/KEK/db, signs the UKI, and emits UEFI-enrolment `.auth` files; the v1.2.2 USB image ships unsigned by default and the operator opts in to signed-boot per machine |
