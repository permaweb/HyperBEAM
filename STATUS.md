...
- Device path proof: with `recorder@1.0,scheduler@1.0` loaded and `hb_message` selected, the inspector rendered `dev_scheduler -> hb_message` as a `1` hop / `43` call device path; clicking the row selected `dev_scheduler` and kept `0` overflow. Screenshot saved to `build/codeviz/validation-device-paths.png`.
- Device path mobile proof: at `390x844`, the same selected `hb_message` view rendered one `310px` device-path row with no horizontally offscreen rows and `0` document/body overflow. Screenshot saved to `build/codeviz/validation-device-paths-mobile.png`.
...
- Added a selected-node device-path inspector section that shows short directed paths from loaded device modules into kernel nodes on the rendered graph.
