...
services to be offered by the node. The base build packages local `dev_*`
namespaces into signed implementation messages and stores them in the configured
`preloaded-store`, although it is possible to add and remove devices as necessary.

### Preloaded Device Store

The following devices are included in the default preloaded device store of a
HyperBEAM node:
