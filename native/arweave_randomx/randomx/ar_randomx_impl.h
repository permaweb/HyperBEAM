...
/* VENDOR: `static'. Upstream declares this at file scope without it, which is
 * harmless there because each RandomX variant is loaded by a separate OS
 * process. HyperBEAM loads rx512, rx4096 and rxsquared into one BEAM, and the
 * libraries are linked `-flat_namespace -undefined suppress', so an external
 * `_stateType' in each makes all three interpose on a single symbol: whichever
 * `load/3' ran last owns the resource type, and every state created before it
 * becomes unreadable ("failed to read state"). That breaks every post-2.9 node,
 * which needs rx4096 for H0 and rxsquared for entropy. */
static ErlNifResourceType* stateType;
