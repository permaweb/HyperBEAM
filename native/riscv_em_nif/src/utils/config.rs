...
use crate::core::state::{deserialize_state, serialize_state, get_state, get_base_path};
...
pub fn init_new_appchain(chain_id: &str) -> bool {
    ...
    if does_exist || genesis_config.len() == 0 {
        ...
        let _ = std::fs::write(format!("{}/{}.json", get_base_path("appchains"), chain_id), serialized_state);
        true
    }
}
