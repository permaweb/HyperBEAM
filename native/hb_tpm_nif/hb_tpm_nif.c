/*
 * HB_TPM_NIF (Native Implemented Functions) Interface
 *
...
#include <stdlib.h>
  ...
  */
static ERL_NIF_TERM nif_check_tpm_support(
    ErlNifEnv* env,
    int argc,
 ...
 */
static ERL_NIF_TERM nif_read_pcr(
    ErlNifEnv* env,
    int argc,
    const ERL_NIF_TERM argv[]
) {
    // Changed from unsigned int to int for proper negative validation
    int pcr_index;
    ...
    if (pcr_index < 0 || pcr_index > 23) {
        ...
        return enif_make_tuple2(env,
            ...
            enif_make_tuple2(
                env,
                enif_make_atom(env, "invalid_pcr_index"),
    ...
    if (init_result != 0) {
        ...
        return enif_make_tuple2(
            env,
            enif_make_atom(env, "error"),
            enif_make_atom(env, "tpm_init_failed")
        );
    }
    ...
    if (result != 0) {
        ...
        return enif_make_tuple2(
            env,
            enif_make_atom(env, "error"),
            enif_make_atom(env, "pcr_read_failed")
        );
    }
 ...
 */
static ERL_NIF_TERM nif_get_random(
    ErlNifEnv* env,
    int argc,
    const ERL_NIF_TERM argv[]
) {
    ...
    if (result != 0) {
        ...
        return enif_make_tuple2(
            env,
            enif_make_atom(env, "error"),
            enif_make_atom(env, "random_failed")
        );
    }
 ...
 */
static ERL_NIF_TERM nif_create_primary_key(
    ErlNifEnv* env,
    int argc,
    const ERL_NIF_TERM argv[]
) {
    ...
    if (result != 0) {
        ...
        return enif_make_tuple2(
            env,
            enif_make_atom(env, "error"),
            enif_make_atom(env, "key_creation_failed")
        );
    }
    ...
    return enif_make_tuple2(
        env,
        enif_make_atom(env, "ok"),
        enif_make_uint(env, handle_uint)
    );
}
 ...
 */
static ERL_NIF_TERM nif_sign_data(
    ErlNifEnv* env,
    int argc,
    const ERL_NIF_TERM argv[]
) {
    ...
    if (key_handle_int <= 0) {
        ...
        return enif_make_tuple2(
            ...
            enif_make_tuple2(
                env,
                enif_make_atom(env, "invalid_key_handle"),
    ...
    if (data_bin.size > TPM_MAX_SIGN_DATA_SIZE) {
        return enif_make_tuple2(
            ...
            enif_make_tuple3(
                env,
                enif_make_atom(env, "data_too_large"),
    ...
    // Buffer for signature (RSA 2048 = ~256 bytes + overhead)
    ...
    if (result != 0) {
        // Signing operation failed - could be invalid key, 
        // authorization, or other TPM error
        return enif_make_tuple2(
            env,
            enif_make_atom(env, "error"),
            enif_make_atom(env, "sign_failed")
        );
    }
 ...
 */
static ERL_NIF_TERM nif_read_clock(
    ErlNifEnv* env,
    int argc,
    const ERL_NIF_TERM argv[]
) {
    ...
    if (result != 0) {
        ...
        return enif_make_tuple2(
            env,
            enif_make_atom(env, "error"),
            enif_make_atom(env, "read_clock_failed")
        );
    }
     ...
     */
    ERL_NIF_TERM clock_info =
        enif_make_tuple4(
            env,
            // Current time
            enif_make_uint64(env, current_time),
            // Reset count
            enif_make_uint(env, reset_count),
            // Restart count
            enif_make_uint(env, restart_count),
            // Safe flag
            enif_make_atom(env, safe ? "true" : "false")
        );
 ...
 */
static ERL_NIF_TERM nif_flush_key(
    ErlNifEnv* env,
    int argc,
    const ERL_NIF_TERM argv[]
) {
    ...
    if (result != 0) {
        ...
        return enif_make_tuple2(
            env,
            enif_make_atom(env, "error"),
            enif_make_atom(env, "flush_failed")
        );
    }
...
static ErlNifFunc nif_funcs[] = {
    // check_tpm_support() -> {ok, boolean()}
    {"check_tpm_support", 0, nif_check_tpm_support, ERL_NIF_DIRTY_JOB_CPU_BOUND},
    // read_pcr_nif(integer()) -> {ok, binary()} | {error, atom()}
    {"read_pcr_nif", 1, nif_read_pcr, ERL_NIF_DIRTY_JOB_CPU_BOUND},
    // get_random_nif(integer()) -> {ok, binary()} | {error, atom()}
    {"get_random_nif", 1, nif_get_random, ERL_NIF_DIRTY_JOB_CPU_BOUND},
    // create_primary_key() -> {ok, integer()} | {error, atom()}
    {"create_primary_key", 0, nif_create_primary_key, ERL_NIF_DIRTY_JOB_CPU_BOUND},
    // sign_data_nif(integer(), binary()) -> {ok, binary()} | {error, atom()}
    {"sign_data_nif", 2, nif_sign_data, ERL_NIF_DIRTY_JOB_CPU_BOUND},
    // read_clock() -> {ok, tuple()} | {error, atom()}
    {"read_clock", 0, nif_read_clock, ERL_NIF_DIRTY_JOB_CPU_BOUND},
    // flush_key_nif(integer()) -> ok | {error, atom()}
    {"flush_key_nif", 1, nif_flush_key, ERL_NIF_DIRTY_JOB_CPU_BOUND}
};
 ...
 */
ERL_NIF_INIT(hb_tpm_nif, nif_funcs, NULL, NULL, NULL, NULL)