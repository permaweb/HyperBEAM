...
public:
	...
	/* VENDOR: workers read and write this concurrently. `volatile' does not
	   synchronize C++ threads, so upstream's declaration has a data race. */
...
/* VENDOR: the kernel the verification driver below hashes with. It defaults to
   _vdf_sha2, so a node that never calls vdf_set_verify_sha2 -- or one whose
   hardware has no SHA-256 extensions -- verifies exactly as upstream does.
   See vdf.h for why this hook exists. */
static vdf_sha2_fn _vdf_sha2_verify = _vdf_sha2;

/* VENDOR: install a verification kernel, but only once it has reproduced
   _vdf_sha2 bit for bit. This is consensus code: a kernel that differed by one
   byte would split the node off the network, so the candidate is checked on
   the machine that is about to run it rather than trusted on the strength of a
   CPU feature bit. The driver only ever calls with checkpointCount == 0 and
   skipCheckpointCount == 0, so the test covers that shape across the iteration
   counts whose loop bounds the kernels special-case, and compares the advanced
   salt as well as the digest -- the driver carries saltBuffer between calls. */
bool vdf_set_verify_sha2(vdf_sha2_fn kernel) {
	static const int hashingIterationsCases[] = {2, 3, 4, 8, 25, 1000};
	const int caseCount = sizeof(hashingIterationsCases)/sizeof(int);

	if (kernel == NULL) {
		return false;
	}
	for(int caseIdx = 0; caseIdx < caseCount; caseIdx++) {
		unsigned char expdSalt[SALT_SIZE], salt[SALT_SIZE];
		unsigned char seed[VDF_SHA_HASH_SIZE];
		unsigned char expdOut[VDF_SHA_HASH_SIZE], out[VDF_SHA_HASH_SIZE];

		for(int i = 0; i < SALT_SIZE; i++) {
			expdSalt[i] = (unsigned char)(1 + 7*i + 31*caseIdx);
			seed[i] = (unsigned char)(2 + 13*i + 5*caseIdx);
		}
		// carry the whole way up the salt, which long_add walks big-endian
		expdSalt[SALT_SIZE-1] = 0xFF;
		memcpy(salt, expdSalt, SALT_SIZE);

		_vdf_sha2(expdSalt, seed, expdOut, NULL, 0, 0, hashingIterationsCases[caseIdx]);
		kernel(salt, seed, out, NULL, 0, 0, hashingIterationsCases[caseIdx]);

		if (0 != memcmp(expdOut, out, VDF_SHA_HASH_SIZE)) {
			return false;
		}
		if (0 != memcmp(expdSalt, salt, SALT_SIZE)) {
			return false;
		}
	}
	_vdf_sha2_verify = kernel;
	return true;
}
...
void _vdf_sha_verify_thread(vdf_sha_verify_thread_arg* _arg) {
	...
	while(true) {
		...
		for(int i=0;i<=arg->job->skipCheckpointCount;i++) {
			/* VENDOR: was _vdf_sha2; hashes with the installed kernel */
			_vdf_sha2_verify(saltBuffer, inCopy, expdOut, NULL, 0, 0, arg->job->hashingIterationsSha);
...
bool vdf_parallel_sha_verify_with_reset(unsigned char* startSaltBuffer, unsigned char* seed, int checkpointCount, int skipCheckpointCount, int hashingIterations, unsigned char* inRes, unsigned char* inCheckpoint, unsigned char* outCheckpoint, unsigned char* resetStepNumberBin256, unsigned char* resetSeed, int maxThreadCount) {
	...
	if (job.verifyRes) {
		...
		for(int i=0;i<=skipCheckpointCount;i++) {
			/* VENDOR: was _vdf_sha2; hashes with the installed kernel */
			_vdf_sha2_verify(finalSaltBuffer, inCopy, expdOut, NULL, 0, 0, hashingIterations);
			memcpy(outFullCheckpoint, expdOut, VDF_SHA_HASH_SIZE);
			outFullCheckpoint += VDF_SHA_HASH_SIZE;
			memcpy(inCopy, expdOut, VDF_SHA_HASH_SIZE);
	...
	for (int checkpointIdx=0;checkpointIdx<checkpointCount;checkpointIdx++) {
		...
		if (_vdf_sha_verify_thread_arg->thread) {
			...
			/* VENDOR: the thread was allocated with `new'. */
