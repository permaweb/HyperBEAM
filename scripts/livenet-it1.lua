...
-- Initialize State
Stakes = Stakes or
    {}                                                            -- Stores all stakes: address -> {amount, stakeTime, cooldownStart, status}
Unstaking = Unstaking or
    {}                                                            -- Stores all unstaking user requests: address -> {msgId -> {amount, release_at}}
UnstakingIndex = UnstakingIndex or
    {}                                                            -- Stores all unstaking requests that are waiting to be processed: timestamp -> array of {user, amount, msgId}
...
Handlers.add('stake', isValidStakeReq, function(msg)
    ...
    assert(msg.Tags["X-TokenPerFailedRequest"] and type(msg.Tags["X-TokenPerFailedRequest"]) == 'string',
        "TokenPerFailedRequest must be a string")
    ...
    assert(msg.Tags["X-MaxPenaltiesPerEpoch"] and type(msg.Tags["X-MaxPenaltiesPerEpoch"]) == 'string',
        "MaxPenaltiesPerEpoch must be a string")
    ...
    assert(msg.Tags["X-MaxRequestCost"] and type(msg.Tags["X-MaxRequestCost"]) == 'string',
        "Maximum Request Cost required and must be a string")
...
Handlers.add('unstake', Handlers.utils.hasMatchingTag("Action", "Unstake"), function(msg)
    ...
    if not UnstakingIndex[release_at] then
        UnstakingIndex[release_at] = {}
    end
    table.insert(UnstakingIndex[release_at], {
        user = msg.From,
        amount = quantity,
        msgId = msg.Id
    })
...
Handlers.add('withdraw', Handlers.utils.hasMatchingTag("Action", "Withdraw"), function(msg)
    ...
    local staked = Stakes[msg.From]
    assert(staked.status == STATUS.IN_COOLDOWN, "unstake first to withdraw")
