...
This accesses the `compute` key on the [`~process@1.0`](../devices/foundational/process-at-1-0.md) device and then navigates to the `cache` key within the resulting state map. Using this path, you will see the latest 'cache' of your process (the number of interactions it has received). Every piece of relevant information about your process can be accessed similarly, effectively providing a native API.

(Note: This represents direct navigation within the process state structure. For accessing data specifically published via the `~patch@1.0` device, see the documentation on [Exposing Process State](https://cookbook_ao.arweave.net/guides/hyperbeam/migration.html#migration-guide-from-legacynet-to-hyperbeam), which typically uses the `/cache/` path.)

### Example 3: Basic `~message@1.0` Usage

Here's a simple example of using [`~message@1.0`](../devices/foundational/message-at-1-0.md) to create a message and retrieve a value:
...
2.  **Root Device:** [`~message@1.0`](../devices/foundational/message-at-1-0.md)
...
The [`~message@1.0`](../devices/foundational/message-at-1-0.md) device can be used to construct and query transient messages, utilizing type casting in query parameters.
