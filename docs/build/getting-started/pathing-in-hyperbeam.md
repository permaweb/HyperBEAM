...
2. Interpret it with the [`~process@1.0`](https://hyperbeam.arweave.net/build/devices/foundational/process-at-1-0.html) device
...
- **Mechanism**: The HyperBEAM node identifies the `+type` suffix (e.g., `+integer`, `+list`, `+map`, `+float`, `+atom`, `+resolve`). It then uses internal functions ([`hb_singleton:maybe_typed`](https://github.com/permaweb/HyperBEAM/blob/edge/src/hb_singleton.erl) and [`dev_codec_structured:decode_value`](https://github.com/permaweb/HyperBEAM/blob/edge/src/dev_codec_structured.erl)) to decode and cast the provided value string into the corresponding Erlang data type before incorporating it into the message.
...
Here's a simple example of using [`~message@1.0`](https://hyperbeam.arweave.net/build/devices/foundational/message-at-1-0.html) to create a message and retrieve a value:
...
2.  **Root Device:** [`~message@1.0`](https://hyperbeam.arweave.net/build/devices/foundational/message-at-1-0.html)
...
The [`~message@1.0`](https://hyperbeam.arweave.net/build/devices/foundational/message-at-1-0.html) device can be used to construct and query transient messages, utilizing type casting in query parameters.
