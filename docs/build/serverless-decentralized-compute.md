...
*   **Execution via Messages:** Sending a message to the process ID triggers the HyperBEAM node (that picks up the message) to:
    ...
    3.  Execute the code using the relevant device ([`dev_wasm`](./devices/source-code/dev_wasm.md) or [`dev_lua`](./devices/source-code/dev_lua.md)), passing the message data and current state.
...
## TEE Attestations (via [`~snp@1.0`](./devices/source-code/dev_snp.md))
...
*   **How it works:** The [`~snp@1.0`](./devices/source-code/dev_snp.md) device interacts with the TEE hardware.
...
*   The HTTP response headers might contain specific signature or attestation data (e.g., using HTTP Message Signatures RFC-9421 via [`dev_codec_httpsig`](./devices/source-code/dev_codec_httpsig.md)).
*   You might query the [`~snp@1.0`](./devices/source-code/dev_snp.md) device directly on the node to get its attestation report.

Refer to documentation on [TEE Nodes](../run/tee-nodes.md) and the [`~snp@1.0`](./devices/source-code/dev_snp.md) device for details.
