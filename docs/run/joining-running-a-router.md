...
!!! note "Using Routers as a Client"
    To use a router as a client, simply make HTTP requests to the router's URL: `https://forward.computer/<process_id>~<device>/<key>...`. The router will automatically route your request to an appropriate worker node.
