...
In HyperBEAM, the `dev_router` device (and associated logic) implements routing functionality. A node configured as a router typically:
...
!!! note "Using Routers as a Client"
    To use a router as a client, simply make HTTP requests to the router's URL: `https://router-1.forward.computer/<process_id>~<device>/<key>...`. The router will automatically route your request to an appropriate worker node.
