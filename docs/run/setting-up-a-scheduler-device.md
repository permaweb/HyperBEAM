...
Run the following curl command on your node host (Ubuntu):
...
> **Note:** The `Url` field should use `https` and point to your node's publicly accessible address (for example, `https://push.forward.computer`). The `nonce` value is generated automatically.
