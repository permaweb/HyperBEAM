document.addEventListener("DOMContentLoaded", function () {
  ...
  const contentVisibilityTargetElement = document.querySelector(
    ".md-main__inner.md-grid"
  ); // Element to hide/show with transition

  if (!header || !paddingTargetElement || !contentVisibilityTargetElement) {
    console.error(
      "Header scroll: Required elements (.md-header, .md-content, or .md-main__inner.md-grid) not found."
    );
    return;
  }
  ...
  function updatePaddings() {
    ...
    if (header.classList.contains("header-hidden")) {
      ...
      if (paddingTargetElement)
        paddingTargetElement.style.paddingTop = headerHeight + "px";
  ...
  window.addEventListener("scroll", function () {
    ...
    if (scrollTop > 0) {
      // When scrolling down / header should be hidden
      if (!header.classList.contains("header-hidden")) {
        header.classList.add("header-hidden");
        updatePaddings();
      }
    } else {
      // When at the top / header should be visible
      if (header.classList.contains("header-hidden")) {
