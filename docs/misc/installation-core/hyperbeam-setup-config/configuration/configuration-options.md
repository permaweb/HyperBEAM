...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `trusted_device_signers` | List | [] | List of device signer addresses or signer policy objects the node should trust; policy objects may set `valid-until-height` and `accepted-packages` |
