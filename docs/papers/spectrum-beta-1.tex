...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\color{Quiet}Spectrum Beta 1}
\fancyhead[R]{\small\color{Quiet}Protocol note}
...
\begin{titlepage}
  \vspace*{0.85in}
  {\color{Spectrum}\rule{0.16in}{1.05in}}\hspace{0.22in}
  \begin{minipage}[b]{0.78\textwidth}
    {\fontsize{28}{32}\selectfont\bfseries\color{Ink}
      Spectrum Beta 1\par}
    \vspace{7pt}
    {\fontsize{16}{20}\selectfont\color{Quiet}
      Probability-Weighted Nametime\\for a Permaweb Namespace\par}
  \end{minipage}

  \vspace{0.7in}
  {\large\bfseries Protocol note}\par
  \vspace{4pt}
  {\color{Quiet}Mathematics, mechanism design, and device architecture}\par

  \vfill
  \begin{minipage}{0.88\textwidth}
  \color{Ink}
  \textbf{Status.} This document specifies \device{spectrum@1.0-beta-1} and
  its pricing composition. It separates normative behavior from calibration
  choices. The beta is deterministic, AR-funded, lease-based, and uses exact
  string probability as its default measure of namespace scarcity.
  \end{minipage}

  \vspace{0.55in}
  {\small\color{Quiet}August 2026}
\end{titlepage}

\section*{Abstract}

Spectrum is a single-market naming protocol. A registration transaction pays
the Arweave network reward and receives the greatest whole number of lease
blocks that payment can buy. The price is not selected by a length table or a
separate auction. Instead, a fixed Markov model assigns every exact name a
probability weight. Active leases consume the corresponding probability mass
over time. A convex bonding curve prices the integral increase in occupied
mass, and future lease expiries divide a quote into constant-occupancy time
slabs. Common exact names therefore consume more of the shared resource per
block; unusual exact names consume less. Expiry releases capacity without a
maintenance transaction.

The protocol is decomposed into three AO-Core devices.
\device{markov@1.0} defines the statistical model.\par
\device{probability-time@1.0} defines the pure pricing function.\par
\device{spectrum@1.0} defines the scheduled registry and lease state machine.\par
This note gives the complete mathematics, mechanism properties, message
interfaces, state invariants, and deployment model for the beta.
...
\section{Exact-name probability}

\subsection{Tokenization and training}
...
\subsection{Likelihood and information measures}
...
\subsection{The natural occurrence knee}
...
\subsection{Deterministic generation}
...
\section{Probability-weighted occupancy}
...
\subsection{Finite-name price per block}
...
\section{Pricing over nametime}

\subsection{Expiry slabs}
...
\begin{itemize}
  ...
  \item \key{/surprisal}, \key{/mean-surprisal}, and \key{/perplexity} derive
        the information measures in Section~3.3 from exact likelihood; and
...
\section{Deployment boundary}\label{sec:deployment}
