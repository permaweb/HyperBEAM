...
\title{Spectrum Beta 1\\
  \large Probability-Weighted Nametime for a Permaweb Namespace}
\author{HyperBEAM contributors}
\date{August 2026}
...
blocks that payment can buy. A fixed Markov model assigns every exact name a
probability weight; active leases consume that probability mass over time. A
convex bonding curve prices the integral increase in occupied mass, and known
expiries divide a quote into constant-occupancy time slabs.
\device{markov@1.0} defines the statistical model,
\device{probability-time@1.0} defines the pure pricing function, and
\device{spectrum@1.0} defines the scheduled registry and lease state machine.
This note specifies their mathematics, mechanism properties, message
interfaces, state invariants, and deployment model.
\end{abstract}
...
\begin{align*}
  S(n) &= -\log_2 p_M(n),\\
  \bar S(n) &= \frac{S(n)}{E},\\
  \operatorname{PP}(n) &= 2^{\bar S(n)},
\end{align*}
where $S$ is total surprisal in bits, $\bar S$ is mean surprisal per scored
event, $\operatorname{PP}$ is perplexity, and $E=L+1$ when end is included.
Pricing uses $p_M(n)$ directly, not its logarithm. Surprisal is the logarithmic
effect size and is useful for analysis, but it does not preserve probability
mass under summation.
...
\begin{table*}[t]
\centering
\caption{Spectrum device composition.}
...
\begin{table*}[t]
\centering
\caption{Spectrum process state.}
