...
[View source on GitHub](https://github.com/permaweb/HyperBEAM/blob/edge/src/dev_query.erl)
...
Return whether a GraphQL esponse in a message has transaction results.
...
Search for the keys specified in the request message.
...
Search the node's store for all of the keys and values in the request,
...
Search the node's store for all of the keys and values in the base
...
Search only for the (list of) key(s) specified in `only` in the request.
...
Match the request against the base message, using the keys to select
...
Return test options with a test store.
...
Search for and find a basic test key.
...
Ensure that we can search for and match only a single key.
...
Ensure that we can specify multiple keys to match.
...
Search for and find a nested test key.
...
Search for and find a list message with typed elements.
...
Ensure user's can opt not to specify a key to resolve, instead specifying
...
Validate the functioning of various return types.
...
*Generated from [dev_query.erl](https://github.com/permaweb/HyperBEAM/blob/edge/src/dev_query.erl)*
