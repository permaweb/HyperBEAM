...
[View source on GitHub](https://github.com/permaweb/HyperBEAM/blob/edge/src/dev_scheduler_cache.erl)

A module that provides a cache for scheduler assignments and locations.
...
A module that provides a cache for scheduler assignments and locations.
Merge the scheduler store with the main store. Used before writing
...
Write the initial assignment message to the cache.
...
Test that a volatile schedule is lost on restart.
...
Test concurrent writes to scheduler store from multiple processes.
...
Test concurrent reads during writes to detect race conditions.
...
Test writing a large volume of assignments to stress memory. Helps
...
Test rapid store restarts under load.
...
Test scheduler store behavior during reset store operations.
...
Test scheduler location operations under stress.
...
Test system behavior with corrupted data in volatile store.
...
*Generated from [dev_scheduler_cache.erl](https://github.com/permaweb/HyperBEAM/blob/edge/src/dev_scheduler_cache.erl)*
