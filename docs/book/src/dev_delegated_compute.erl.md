...
[View source on GitHub](https://github.com/permaweb/HyperBEAM/blob/edge/src/dev_delegated_compute.erl)
...
We assume that the compute engine stores its own internal state,
Call the delegated server to compute the result. The endpoint is
...
Execute dry-run computation on a remote machine via relay and use
...
Extract the JSON response from the delegated compute response.
...
### handle_relay_response

Handle the response from the delegated compute server. Assumes that the
...
Generate a snapshot of a running computation by calling the 
...
*Generated from [dev_delegated_compute.erl](https://github.com/permaweb/HyperBEAM/blob/edge/src/dev_delegated_compute.erl)*
