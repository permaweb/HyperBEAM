...
[View source on GitHub](https://github.com/permaweb/HyperBEAM/blob/edge/src/dev_genesis_wasm.erl)
...
Genesis-wasm device compute handler.
...
Snapshot the state of the process via the `delegated-compute@1.0` device.
...
Proxy a request to the delegated-compute@1.0 device, ensuring that
...
Handle normal compute execution with state persistence (GET method).
...
### is_genesis_wasm_server_running
...
*Generated from [dev_genesis_wasm.erl](https://github.com/permaweb/HyperBEAM/blob/edge/src/dev_genesis_wasm.erl)*
